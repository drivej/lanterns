import { jsx as un } from "react/jsx-runtime";
import { useState as ur, useRef as Pe, useEffect as Fe, useCallback as gn } from "react";
var De = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, ze, Wi;
function An() {
  if (Wi) return ze;
  Wi = 1;
  var o = "Expected a function", t = NaN, e = "[object Symbol]", i = /^\s+|\s+$/g, r = /^[-+]0x[0-9a-f]+$/i, n = /^0b[01]+$/i, a = /^0o[0-7]+$/i, s = parseInt, h = typeof De == "object" && De && De.Object === Object && De, u = typeof self == "object" && self && self.Object === Object && self, l = h || u || Function("return this")(), g = Object.prototype, A = g.toString, S = Math.max, p = Math.min, k = function() {
    return l.Date.now();
  };
  function I(c, J, m) {
    var Q, y, B, x, v, O, P = 0, H = !1, U = !1, F = !0;
    if (typeof c != "function")
      throw new TypeError(o);
    J = d(J) || 0, w(m) && (H = !!m.leading, U = "maxWait" in m, B = U ? S(d(m.maxWait) || 0, J) : B, F = "trailing" in m ? !!m.trailing : F);
    function V(L) {
      var q = Q, Pt = y;
      return Q = y = void 0, P = L, x = c.apply(Pt, q), x;
    }
    function et(L) {
      return P = L, v = setTimeout(tt, J), H ? V(L) : x;
    }
    function mt(L) {
      var q = L - O, Pt = L - P, Y = J - q;
      return U ? p(Y, B - Pt) : Y;
    }
    function ft(L) {
      var q = L - O, Pt = L - P;
      return O === void 0 || q >= J || q < 0 || U && Pt >= B;
    }
    function tt() {
      var L = k();
      if (ft(L))
        return ut(L);
      v = setTimeout(tt, mt(L));
    }
    function ut(L) {
      return v = void 0, F && Q ? V(L) : (Q = y = void 0, x);
    }
    function it() {
      v !== void 0 && clearTimeout(v), P = 0, Q = O = y = v = void 0;
    }
    function It() {
      return v === void 0 ? x : ut(k());
    }
    function Vt() {
      var L = k(), q = ft(L);
      if (Q = arguments, y = this, O = L, q) {
        if (v === void 0)
          return et(O);
        if (U)
          return v = setTimeout(tt, J), V(O);
      }
      return v === void 0 && (v = setTimeout(tt, J)), x;
    }
    return Vt.cancel = it, Vt.flush = It, Vt;
  }
  function w(c) {
    var J = typeof c;
    return !!c && (J == "object" || J == "function");
  }
  function f(c) {
    return !!c && typeof c == "object";
  }
  function C(c) {
    return typeof c == "symbol" || f(c) && A.call(c) == e;
  }
  function d(c) {
    if (typeof c == "number")
      return c;
    if (C(c))
      return t;
    if (w(c)) {
      var J = typeof c.valueOf == "function" ? c.valueOf() : c;
      c = w(J) ? J + "" : J;
    }
    if (typeof c != "string")
      return c === 0 ? c : +c;
    c = c.replace(i, "");
    var m = n.test(c);
    return m || a.test(c) ? s(c.slice(2), m ? 2 : 8) : r.test(c) ? t : +c;
  }
  return ze = I, ze;
}
An();
function kn() {
  const o = Pe(!1);
  return Fe(() => (o.current = !0, () => {
    o.current = !1;
  }), []), gn(() => o.current, []);
}
var Ni = {
  width: void 0,
  height: void 0
};
function cn(o) {
  const { ref: t, box: e = "content-box" } = o, [{ width: i, height: r }, n] = ur(Ni), a = kn(), s = Pe({ ...Ni }), h = Pe(void 0);
  return h.current = o.onResize, Fe(() => {
    if (!t.current || typeof window > "u" || !("ResizeObserver" in window))
      return;
    const u = new ResizeObserver(([l]) => {
      const g = e === "border-box" ? "borderBoxSize" : e === "device-pixel-content-box" ? "devicePixelContentBoxSize" : "contentBoxSize", A = Gi(l, g, "inlineSize"), S = Gi(l, g, "blockSize");
      if (s.current.width !== A || s.current.height !== S) {
        const k = { width: A, height: S };
        s.current.width = A, s.current.height = S, h.current ? h.current(k) : a() && n(k);
      }
    });
    return u.observe(t.current, { box: e }), () => {
      u.disconnect();
    };
  }, [e, t, a]), { width: i, height: r };
}
function Gi(o, t, e) {
  return o[t] ? Array.isArray(o[t]) ? o[t][0][e] : (
    // @ts-ignore Support Firefox's non-standard behavior
    o[t][e]
  ) : t === "contentBoxSize" ? o.contentRect[e === "inlineSize" ? "width" : "height"] : void 0;
}
function Ze(o, t, e) {
  return t < e ? Math.max(t, Math.min(e, o)) : Math.max(e, Math.min(t, o));
}
const Vi = Math.PI / 180;
function Sn(o, t, e, i, r, n, a) {
  i < 2 * n && (n = i / 2), r < 2 * n && (n = r / 2), o.beginPath(), o.moveTo(t + n, e), o.arcTo(t + i, e, t + i, e + r, n), o.arcTo(t + i, e + r, t, e + r, n), o.arcTo(t, e + r, t, e, n), o.arcTo(t, e, t + i, e, n), o.closePath(), a && (o.fillStyle = a, o.fill());
}
class qi {
  /**
   * @param {any} config
   */
  constructor(t) {
    var e = this;
    this.minX = -1 / 0, this.maxX = 1 / 0, this.minY = -1 / 0, this.maxY = 1 / 0, this.minZ = -1 / 0, this.maxZ = 1 / 0, this.set = function(i) {
      for (var r in i)
        this[r] = i[r];
    }, this.clamp = function(i) {
      return "x" in i && (i.x = Ze(i.x, e.minX, e.maxX)), "y" in i && (i.y = Ze(i.y, e.minY, e.maxY)), "z" in i && (i.z = Ze(i.z, e.minZ, e.maxZ)), i;
    }, this.set(t);
  }
}
function xt(o) {
  if (o === void 0)
    throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
  return o;
}
function gr(o, t) {
  o.prototype = Object.create(t.prototype), o.prototype.constructor = o, o.__proto__ = t;
}
/*!
 * GSAP 3.15.0
 * https://gsap.com
 *
 * @license Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var St = {
  autoSleep: 120,
  force3D: "auto",
  nullTargetWarn: 1,
  units: {
    lineHeight: ""
  }
}, pe = {
  duration: 0.5,
  overwrite: !1,
  delay: 0
}, Ji, X, R, Jt = 1e8, E = 1 / Jt, si = Math.PI * 2, dn = si / 4, pn = 0, Ar = Math.sqrt, In = Math.cos, Cn = Math.sin, j = function(t) {
  return typeof t == "string";
}, M = function(t) {
  return typeof t == "function";
}, Kt = function(t) {
  return typeof t == "number";
}, wi = function(t) {
  return typeof t > "u";
}, Dt = function(t) {
  return typeof t == "object";
}, at = function(t) {
  return t !== !1;
}, mi = function() {
  return typeof window < "u";
}, ve = function(t) {
  return M(t) || j(t);
}, kr = typeof ArrayBuffer == "function" && ArrayBuffer.isView || function() {
}, $ = Array.isArray, Jn = /random\([^)]+\)/g, wn = /,\s*/g, Yi = /(?:-?\.?\d|\.)+/gi, cr = /[-+=.]*\d+[.e\-+]*\d*[e\-+]*\d*/g, ee = /[-+=.]*\d+[.e-]*\d*[a-z%]*/g, je = /[-+=.]*\d+\.?\d*(?:e-|e\+)?\d*/gi, Sr = /[+-]=-?[.\d]+/, mn = /[^,'"\[\]\s]+/gi, yn = /^[+\-=e\s\d]*\d+[.\d]*([a-z]*|%)\s*$/i, N, yt, oi, yi, dt = {}, He = {}, dr, pr = function(t) {
  return (He = se(t, dt)) && lt;
}, Qi = function(t, e) {
  return console.warn("Invalid property", t, "set to", e, "Missing plugin? gsap.registerPlugin()");
}, Ie = function(t, e) {
  return !e && console.warn(t);
}, Ir = function(t, e) {
  return t && (dt[t] = e) && He && (He[t] = e) || dt;
}, Ce = function() {
  return 0;
}, Qn = {
  suppressEvents: !0,
  isStart: !0,
  kill: !1
}, xe = {
  suppressEvents: !0,
  kill: !1
}, Bn = {
  suppressEvents: !0
}, Bi = {}, Et = [], hi = {}, Cr, gt = {}, Xe = {}, bi = 30, Oe = [], Di = "", vi = function(t) {
  var e = t[0], i, r;
  if (Dt(e) || M(e) || (t = [t]), !(i = (e._gsap || {}).harness)) {
    for (r = Oe.length; r-- && !Oe[r].targetTest(e); )
      ;
    i = Oe[r];
  }
  for (r = t.length; r--; )
    t[r] && (t[r]._gsap || (t[r]._gsap = new Gr(t[r], i))) || t.splice(r, 1);
  return t;
}, zt = function(t) {
  return t._gsap || vi(wt(t))[0]._gsap;
}, Jr = function(t, e, i) {
  return (i = t[e]) && M(i) ? t[e]() : wi(i) && t.getAttribute && t.getAttribute(e) || i;
}, st = function(t, e) {
  return (t = t.split(",")).forEach(e) || t;
}, z = function(t) {
  return Math.round(t * 1e5) / 1e5 || 0;
}, W = function(t) {
  return Math.round(t * 1e7) / 1e7 || 0;
}, re = function(t, e) {
  var i = e.charAt(0), r = parseFloat(e.substr(2));
  return t = parseFloat(t), i === "+" ? t + r : i === "-" ? t - r : i === "*" ? t * r : t / r;
}, Dn = function(t, e) {
  for (var i = e.length, r = 0; t.indexOf(e[r]) < 0 && ++r < i; )
    ;
  return r < i;
}, Te = function() {
  var t = Et.length, e = Et.slice(0), i, r;
  for (hi = {}, Et.length = 0, i = 0; i < t; i++)
    r = e[i], r && r._lazy && (r.render(r._lazy[0], r._lazy[1], !0)._lazy = 0);
}, xi = function(t) {
  return !!(t._initted || t._startAt || t.add);
}, wr = function(t, e, i, r) {
  Et.length && !X && Te(), t.render(e, i, !!(X && e < 0 && xi(t))), Et.length && !X && Te();
}, mr = function(t) {
  var e = parseFloat(t);
  return (e || e === 0) && (t + "").match(mn).length < 2 ? e : j(t) ? t.trim() : t;
}, yr = function(t) {
  return t;
}, pt = function(t, e) {
  for (var i in e)
    i in t || (t[i] = e[i]);
  return t;
}, vn = function(t) {
  return function(e, i) {
    for (var r in i)
      r in e || r === "duration" && t || r === "ease" || (e[r] = i[r]);
  };
}, se = function(t, e) {
  for (var i in e)
    t[i] = e[i];
  return t;
}, Mi = function o(t, e) {
  for (var i in e)
    i !== "__proto__" && i !== "constructor" && i !== "prototype" && (t[i] = Dt(e[i]) ? o(t[i] || (t[i] = {}), e[i]) : e[i]);
  return t;
}, Ee = function(t, e) {
  var i = {}, r;
  for (r in t)
    r in e || (i[r] = t[r]);
  return i;
}, ce = function(t) {
  var e = t.parent || N, i = t.keyframes ? vn($(t.keyframes)) : pt;
  if (at(t.inherit))
    for (; e; )
      i(t, e.vars.defaults), e = e.parent || e._dp;
  return t;
}, xn = function(t, e) {
  for (var i = t.length, r = i === e.length; r && i-- && t[i] === e[i]; )
    ;
  return i < 0;
}, Qr = function(t, e, i, r, n) {
  var a = t[r], s;
  if (n)
    for (s = e[n]; a && a[n] > s; )
      a = a._prev;
  return a ? (e._next = a._next, a._next = e) : (e._next = t[i], t[i] = e), e._next ? e._next._prev = e : t[r] = e, e._prev = a, e.parent = e._dp = t, e;
}, Ve = function(t, e, i, r) {
  i === void 0 && (i = "_first"), r === void 0 && (r = "_last");
  var n = e._prev, a = e._next;
  n ? n._next = a : t[i] === e && (t[i] = a), a ? a._prev = n : t[r] === e && (t[r] = n), e._next = e._prev = e.parent = null;
}, Lt = function(t, e) {
  t.parent && (!e || t.parent.autoRemoveChildren) && t.parent.remove && t.parent.remove(t), t._act = 0;
}, Zt = function(t, e) {
  if (t && (!e || e._end > t._dur || e._start < 0))
    for (var i = t; i; )
      i._dirty = 1, i = i.parent;
  return t;
}, On = function(t) {
  for (var e = t.parent; e && e.parent; )
    e._dirty = 1, e.totalDuration(), e = e.parent;
  return t;
}, li = function(t, e, i, r) {
  return t._startAt && (X ? t._startAt.revert(xe) : t.vars.immediateRender && !t.vars.autoRevert || t._startAt.render(e, !0, r));
}, Kn = function o(t) {
  return !t || t._ts && o(t.parent);
}, zi = function(t) {
  return t._repeat ? oe(t._tTime, t = t.duration() + t._rDelay) * t : 0;
}, oe = function(t, e) {
  var i = Math.floor(t = W(t / e));
  return t && i === t ? i - 1 : i;
}, Re = function(t, e) {
  return (t - e._start) * e._ts + (e._ts >= 0 ? 0 : e._dirty ? e.totalDuration() : e._tDur);
}, qe = function(t) {
  return t._end = W(t._start + (t._tDur / Math.abs(t._ts || t._rts || E) || 0));
}, Ye = function(t, e) {
  var i = t._dp;
  return i && i.smoothChildTiming && t._ts && (t._start = W(i._time - (t._ts > 0 ? e / t._ts : ((t._dirty ? t.totalDuration() : t._tDur) - e) / -t._ts)), qe(t), i._dirty || Zt(i, t)), t;
}, Br = function(t, e) {
  var i;
  if ((e._time || !e._dur && e._initted || e._start < t._time && (e._dur || !e.add)) && (i = Re(t.rawTime(), e), (!e._dur || Be(0, e.totalDuration(), i) - e._tTime > E) && e.render(i, !0)), Zt(t, e)._dp && t._initted && t._time >= t._dur && t._ts) {
    if (t._dur < t.duration())
      for (i = t; i._dp; )
        i.rawTime() >= 0 && i.totalTime(i._tTime), i = i._dp;
    t._zTime = -E;
  }
}, Qt = function(t, e, i, r) {
  return e.parent && Lt(e), e._start = W((Kt(i) ? i : i || t !== N ? Ct(t, i, e) : t._time) + e._delay), e._end = W(e._start + (e.totalDuration() / Math.abs(e.timeScale()) || 0)), Qr(t, e, "_first", "_last", t._sort ? "_start" : 0), fi(e) || (t._recent = e), r || Br(t, e), t._ts < 0 && Ye(t, t._tTime), t;
}, Dr = function(t, e) {
  return (dt.ScrollTrigger || Qi("scrollTrigger", e)) && dt.ScrollTrigger.create(e, t);
}, vr = function(t, e, i, r, n) {
  if (Ki(t, e, n), !t._initted)
    return 1;
  if (!i && t._pt && !X && (t._dur && t.vars.lazy !== !1 || !t._dur && t.vars.lazy) && Cr !== At.frame)
    return Et.push(t), t._lazy = [n, r], 1;
}, Un = function o(t) {
  var e = t.parent;
  return e && e._ts && e._initted && !e._lock && (e.rawTime() < 0 || o(e));
}, fi = function(t) {
  var e = t.data;
  return e === "isFromStart" || e === "isStart";
}, Pn = function(t, e, i, r) {
  var n = t.ratio, a = e < 0 || !e && (!t._start && Un(t) && !(!t._initted && fi(t)) || (t._ts < 0 || t._dp._ts < 0) && !fi(t)) ? 0 : 1, s = t._rDelay, h = 0, u, l, g;
  if (s && t._repeat && (h = Be(0, t._tDur, e), l = oe(h, s), t._yoyo && l & 1 && (a = 1 - a), l !== oe(t._tTime, s) && (n = 1 - a, t.vars.repeatRefresh && t._initted && t.invalidate())), a !== n || X || r || t._zTime === E || !e && t._zTime) {
    if (!t._initted && vr(t, e, r, i, h))
      return;
    for (g = t._zTime, t._zTime = e || (i ? E : 0), i || (i = e && !g), t.ratio = a, t._from && (a = 1 - a), t._time = 0, t._tTime = h, u = t._pt; u; )
      u.r(a, u.d), u = u._next;
    e < 0 && li(t, e, i, !0), t._onUpdate && !i && kt(t, "onUpdate"), h && t._repeat && !i && t.parent && kt(t, "onRepeat"), (e >= t._tDur || e < 0) && t.ratio === a && (a && Lt(t, 1), !i && !X && (kt(t, a ? "onComplete" : "onReverseComplete", !0), t._prom && t._prom()));
  } else t._zTime || (t._zTime = e);
}, Fn = function(t, e, i) {
  var r;
  if (i > e)
    for (r = t._first; r && r._start <= i; ) {
      if (r.data === "isPause" && r._start > e)
        return r;
      r = r._next;
    }
  else
    for (r = t._last; r && r._start >= i; ) {
      if (r.data === "isPause" && r._start < e)
        return r;
      r = r._prev;
    }
}, he = function(t, e, i, r) {
  var n = t._repeat, a = W(e) || 0, s = t._tTime / t._tDur;
  return s && !r && (t._time *= a / t._dur), t._dur = a, t._tDur = n ? n < 0 ? 1e10 : W(a * (n + 1) + t._rDelay * n) : a, s > 0 && !r && Ye(t, t._tTime = t._tDur * s), t.parent && qe(t), i || Zt(t.parent, t), t;
}, Zi = function(t) {
  return t instanceof rt ? Zt(t) : he(t, t._dur);
}, Hn = {
  _start: 0,
  endTime: Ce,
  totalDuration: Ce
}, Ct = function o(t, e, i) {
  var r = t.labels, n = t._recent || Hn, a = t.duration() >= Jt ? n.endTime(!1) : t._dur, s, h, u;
  return j(e) && (isNaN(e) || e in r) ? (h = e.charAt(0), u = e.substr(-1) === "%", s = e.indexOf("="), h === "<" || h === ">" ? (s >= 0 && (e = e.replace(/=/, "")), (h === "<" ? n._start : n.endTime(n._repeat >= 0)) + (parseFloat(e.substr(1)) || 0) * (u ? (s < 0 ? n : i).totalDuration() / 100 : 1)) : s < 0 ? (e in r || (r[e] = a), r[e]) : (h = parseFloat(e.charAt(s - 1) + e.substr(s + 1)), u && i && (h = h / 100 * ($(i) ? i[0] : i).totalDuration()), s > 1 ? o(t, e.substr(0, s - 1), i) + h : a + h)) : e == null ? a : +e;
}, Se = function(t, e, i) {
  var r = Kt(e[1]), n = (r ? 2 : 1) + (t < 2 ? 0 : 1), a = e[n], s, h;
  if (r && (a.duration = e[1]), a.parent = i, t) {
    for (s = a, h = i; h && !("immediateRender" in s); )
      s = h.vars.defaults || {}, h = at(h.vars.inherit) && h.parent;
    a.immediateRender = at(s.immediateRender), t < 2 ? a.runBackwards = 1 : a.startAt = e[n - 1];
  }
  return new Z(e[0], a, e[n + 1]);
}, Gt = function(t, e) {
  return t || t === 0 ? e(t) : e;
}, Be = function(t, e, i) {
  return i < t ? t : i > e ? e : i;
}, _ = function(t, e) {
  return !j(t) || !(e = yn.exec(t)) ? "" : e[1];
}, Tn = function(t, e, i) {
  return Gt(i, function(r) {
    return Be(t, e, r);
  });
}, ui = [].slice, xr = function(t, e) {
  return t && Dt(t) && "length" in t && (!e && !t.length || t.length - 1 in t && Dt(t[0])) && !t.nodeType && t !== yt;
}, En = function(t, e, i) {
  return i === void 0 && (i = []), t.forEach(function(r) {
    var n;
    return j(r) && !e || xr(r, 1) ? (n = i).push.apply(n, wt(r)) : i.push(r);
  }) || i;
}, wt = function(t, e, i) {
  return R && !e && R.selector ? R.selector(t) : j(t) && !i && (oi || !le()) ? ui.call((e || yi).querySelectorAll(t), 0) : $(t) ? En(t, i) : xr(t) ? ui.call(t, 0) : t ? [t] : [];
}, gi = function(t) {
  return t = wt(t)[0] || Ie("Invalid scope") || {}, function(e) {
    var i = t.current || t.nativeElement || t;
    return wt(e, i.querySelectorAll ? i : i === t ? Ie("Invalid scope") || yi.createElement("div") : t);
  };
}, Or = function(t) {
  return t.sort(function() {
    return 0.5 - Math.random();
  });
}, Kr = function(t) {
  if (M(t))
    return t;
  var e = Dt(t) ? t : {
    each: t
  }, i = jt(e.ease), r = e.from || 0, n = parseFloat(e.base) || 0, a = {}, s = r > 0 && r < 1, h = isNaN(r) || s, u = e.axis, l = r, g = r;
  return j(r) ? l = g = {
    center: 0.5,
    edges: 0.5,
    end: 1
  }[r] || 0 : !s && h && (l = r[0], g = r[1]), function(A, S, p) {
    var k = (p || e).length, I = a[k], w, f, C, d, c, J, m, Q, y;
    if (!I) {
      if (y = e.grid === "auto" ? 0 : (e.grid || [1, Jt])[1], !y) {
        for (m = -Jt; m < (m = p[y++].getBoundingClientRect().left) && y < k; )
          ;
        y < k && y--;
      }
      for (I = a[k] = [], w = h ? Math.min(y, k) * l - 0.5 : r % y, f = y === Jt ? 0 : h ? k * g / y - 0.5 : r / y | 0, m = 0, Q = Jt, J = 0; J < k; J++)
        C = J % y - w, d = f - (J / y | 0), I[J] = c = u ? Math.abs(u === "y" ? d : C) : Ar(C * C + d * d), c > m && (m = c), c < Q && (Q = c);
      r === "random" && Or(I), I.max = m - Q, I.min = Q, I.v = k = (parseFloat(e.amount) || parseFloat(e.each) * (y > k ? k - 1 : u ? u === "y" ? k / y : y : Math.max(y, k / y)) || 0) * (r === "edges" ? -1 : 1), I.b = k < 0 ? n - k : n, I.u = _(e.amount || e.each) || 0, i = i && k < 0 ? jn(i) : i;
    }
    return k = (I[A] - I.min) / I.max || 0, W(I.b + (i ? i(k) : k) * I.v) + I.u;
  };
}, Ai = function(t) {
  var e = Math.pow(10, ((t + "").split(".")[1] || "").length);
  return function(i) {
    var r = W(Math.round(parseFloat(i) / t) * t * e);
    return (r - r % 1) / e + (Kt(i) ? 0 : _(i));
  };
}, Ur = function(t, e) {
  var i = $(t), r, n;
  return !i && Dt(t) && (r = i = t.radius || Jt, t.values ? (t = wt(t.values), (n = !Kt(t[0])) && (r *= r)) : t = Ai(t.increment)), Gt(e, i ? M(t) ? function(a) {
    return n = t(a), Math.abs(n - a) <= r ? n : a;
  } : function(a) {
    for (var s = parseFloat(n ? a.x : a), h = parseFloat(n ? a.y : 0), u = Jt, l = 0, g = t.length, A, S; g--; )
      n ? (A = t[g].x - s, S = t[g].y - h, A = A * A + S * S) : A = Math.abs(t[g] - s), A < u && (u = A, l = g);
    return l = !r || u <= r ? t[l] : a, n || l === a || Kt(a) ? l : l + _(a);
  } : Ai(t));
}, Pr = function(t, e, i, r) {
  return Gt($(t) ? !e : i === !0 ? !!(i = 0) : !r, function() {
    return $(t) ? t[~~(Math.random() * t.length)] : (i = i || 1e-5) && (r = i < 1 ? Math.pow(10, (i + "").length - 2) : 1) && Math.floor(Math.round((t - i / 2 + Math.random() * (e - t + i * 0.99)) / i) * i * r) / r;
  });
}, Rn = function() {
  for (var t = arguments.length, e = new Array(t), i = 0; i < t; i++)
    e[i] = arguments[i];
  return function(r) {
    return e.reduce(function(n, a) {
      return a(n);
    }, r);
  };
}, Ln = function(t, e) {
  return function(i) {
    return t(parseFloat(i)) + (e || _(i));
  };
}, Wn = function(t, e, i) {
  return Hr(t, e, 0, 1, i);
}, Fr = function(t, e, i) {
  return Gt(i, function(r) {
    return t[~~e(r)];
  });
}, Nn = function o(t, e, i) {
  var r = e - t;
  return $(t) ? Fr(t, o(0, t.length), e) : Gt(i, function(n) {
    return (r + (n - t) % r) % r + t;
  });
}, Gn = function o(t, e, i) {
  var r = e - t, n = r * 2;
  return $(t) ? Fr(t, o(0, t.length - 1), e) : Gt(i, function(a) {
    return a = (n + (a - t) % n) % n || 0, t + (a > r ? n - a : a);
  });
}, Je = function(t) {
  return t.replace(Jn, function(e) {
    var i = e.indexOf("[") + 1, r = e.substring(i || 7, i ? e.indexOf("]") : e.length - 1).split(wn);
    return Pr(i ? r : +r[0], i ? 0 : +r[1], +r[2] || 1e-5);
  });
}, Hr = function(t, e, i, r, n) {
  var a = e - t, s = r - i;
  return Gt(n, function(h) {
    return i + ((h - t) / a * s || 0);
  });
}, Vn = function o(t, e, i, r) {
  var n = isNaN(t + e) ? 0 : function(S) {
    return (1 - S) * t + S * e;
  };
  if (!n) {
    var a = j(t), s = {}, h, u, l, g, A;
    if (i === !0 && (r = 1) && (i = null), a)
      t = {
        p: t
      }, e = {
        p: e
      };
    else if ($(t) && !$(e)) {
      for (l = [], g = t.length, A = g - 2, u = 1; u < g; u++)
        l.push(o(t[u - 1], t[u]));
      g--, n = function(p) {
        p *= g;
        var k = Math.min(A, ~~p);
        return l[k](p - k);
      }, i = e;
    } else r || (t = se($(t) ? [] : {}, t));
    if (!l) {
      for (h in e)
        Oi.call(s, t, h, "get", e[h]);
      n = function(p) {
        return Fi(p, s) || (a ? t.p : t);
      };
    }
  }
  return Gt(i, n);
}, ji = function(t, e, i) {
  var r = t.labels, n = Jt, a, s, h;
  for (a in r)
    s = r[a] - e, s < 0 == !!i && s && n > (s = Math.abs(s)) && (h = a, n = s);
  return h;
}, kt = function(t, e, i) {
  var r = t.vars, n = r[e], a = R, s = t._ctx, h, u, l;
  if (n)
    return h = r[e + "Params"], u = r.callbackScope || t, i && Et.length && Te(), s && (R = s), l = h ? n.apply(u, h) : n.call(u), R = a, l;
}, ge = function(t) {
  return Lt(t), t.scrollTrigger && t.scrollTrigger.kill(!!X), t.progress() < 1 && kt(t, "onInterrupt"), t;
}, ie, Tr = [], Er = function(t) {
  if (t)
    if (t = !t.name && t.default || t, mi() || t.headless) {
      var e = t.name, i = M(t), r = e && !i && t.init ? function() {
        this._props = [];
      } : t, n = {
        init: Ce,
        render: Fi,
        add: Oi,
        kill: sa,
        modifier: aa,
        rawVars: 0
      }, a = {
        targetTest: 0,
        get: 0,
        getSetter: Pi,
        aliases: {},
        register: 0
      };
      if (le(), t !== r) {
        if (gt[e])
          return;
        pt(r, pt(Ee(t, n), a)), se(r.prototype, se(n, Ee(t, a))), gt[r.prop = e] = r, t.targetTest && (Oe.push(r), Bi[e] = 1), e = (e === "css" ? "CSS" : e.charAt(0).toUpperCase() + e.substr(1)) + "Plugin";
      }
      Ir(e, r), t.register && t.register(lt, r, ot);
    } else
      Tr.push(t);
}, T = 255, Ae = {
  aqua: [0, T, T],
  lime: [0, T, 0],
  silver: [192, 192, 192],
  black: [0, 0, 0],
  maroon: [128, 0, 0],
  teal: [0, 128, 128],
  blue: [0, 0, T],
  navy: [0, 0, 128],
  white: [T, T, T],
  olive: [128, 128, 0],
  yellow: [T, T, 0],
  orange: [T, 165, 0],
  gray: [128, 128, 128],
  purple: [128, 0, 128],
  green: [0, 128, 0],
  red: [T, 0, 0],
  pink: [T, 192, 203],
  cyan: [0, T, T],
  transparent: [T, T, T, 0]
}, _e = function(t, e, i) {
  return t += t < 0 ? 1 : t > 1 ? -1 : 0, (t * 6 < 1 ? e + (i - e) * t * 6 : t < 0.5 ? i : t * 3 < 2 ? e + (i - e) * (2 / 3 - t) * 6 : e) * T + 0.5 | 0;
}, Rr = function(t, e, i) {
  var r = t ? Kt(t) ? [t >> 16, t >> 8 & T, t & T] : 0 : Ae.black, n, a, s, h, u, l, g, A, S, p;
  if (!r) {
    if (t.substr(-1) === "," && (t = t.substr(0, t.length - 1)), Ae[t])
      r = Ae[t];
    else if (t.charAt(0) === "#") {
      if (t.length < 6 && (n = t.charAt(1), a = t.charAt(2), s = t.charAt(3), t = "#" + n + n + a + a + s + s + (t.length === 5 ? t.charAt(4) + t.charAt(4) : "")), t.length === 9)
        return r = parseInt(t.substr(1, 6), 16), [r >> 16, r >> 8 & T, r & T, parseInt(t.substr(7), 16) / 255];
      t = parseInt(t.substr(1), 16), r = [t >> 16, t >> 8 & T, t & T];
    } else if (t.substr(0, 3) === "hsl") {
      if (r = p = t.match(Yi), !e)
        h = +r[0] % 360 / 360, u = +r[1] / 100, l = +r[2] / 100, a = l <= 0.5 ? l * (u + 1) : l + u - l * u, n = l * 2 - a, r.length > 3 && (r[3] *= 1), r[0] = _e(h + 1 / 3, n, a), r[1] = _e(h, n, a), r[2] = _e(h - 1 / 3, n, a);
      else if (~t.indexOf("="))
        return r = t.match(cr), i && r.length < 4 && (r[3] = 1), r;
    } else
      r = t.match(Yi) || Ae.transparent;
    r = r.map(Number);
  }
  return e && !p && (n = r[0] / T, a = r[1] / T, s = r[2] / T, g = Math.max(n, a, s), A = Math.min(n, a, s), l = (g + A) / 2, g === A ? h = u = 0 : (S = g - A, u = l > 0.5 ? S / (2 - g - A) : S / (g + A), h = g === n ? (a - s) / S + (a < s ? 6 : 0) : g === a ? (s - n) / S + 2 : (n - a) / S + 4, h *= 60), r[0] = ~~(h + 0.5), r[1] = ~~(u * 100 + 0.5), r[2] = ~~(l * 100 + 0.5)), i && r.length < 4 && (r[3] = 1), r;
}, Lr = function(t) {
  var e = [], i = [], r = -1;
  return t.split(Rt).forEach(function(n) {
    var a = n.match(ee) || [];
    e.push.apply(e, a), i.push(r += a.length + 1);
  }), e.c = i, e;
}, Xi = function(t, e, i) {
  var r = "", n = (t + r).match(Rt), a = e ? "hsla(" : "rgba(", s = 0, h, u, l, g;
  if (!n)
    return t;
  if (n = n.map(function(A) {
    return (A = Rr(A, e, 1)) && a + (e ? A[0] + "," + A[1] + "%," + A[2] + "%," + A[3] : A.join(",")) + ")";
  }), i && (l = Lr(t), h = i.c, h.join(r) !== l.c.join(r)))
    for (u = t.replace(Rt, "1").split(ee), g = u.length - 1; s < g; s++)
      r += u[s] + (~h.indexOf(s) ? n.shift() || a + "0,0,0,0)" : (l.length ? l : n.length ? n : i).shift());
  if (!u)
    for (u = t.split(Rt), g = u.length - 1; s < g; s++)
      r += u[s] + n[s];
  return r + u[g];
}, Rt = (function() {
  var o = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", t;
  for (t in Ae)
    o += "|" + t + "\\b";
  return new RegExp(o + ")", "gi");
})(), qn = /hsl[a]?\(/, Wr = function(t) {
  var e = t.join(" "), i;
  if (Rt.lastIndex = 0, Rt.test(e))
    return i = qn.test(e), t[1] = Xi(t[1], i), t[0] = Xi(t[0], i, Lr(t[1])), !0;
}, we, At = (function() {
  var o = Date.now, t = 500, e = 33, i = o(), r = i, n = 1e3 / 240, a = n, s = [], h, u, l, g, A, S, p = function k(I) {
    var w = o() - r, f = I === !0, C, d, c, J;
    if ((w > t || w < 0) && (i += w - e), r += w, c = r - i, C = c - a, (C > 0 || f) && (J = ++g.frame, A = c - g.time * 1e3, g.time = c = c / 1e3, a += C + (C >= n ? 4 : n - C), d = 1), f || (h = u(k)), d)
      for (S = 0; S < s.length; S++)
        s[S](c, A, J, I);
  };
  return g = {
    time: 0,
    frame: 0,
    tick: function() {
      p(!0);
    },
    deltaRatio: function(I) {
      return A / (1e3 / (I || 60));
    },
    wake: function() {
      dr && (!oi && mi() && (yt = oi = window, yi = yt.document || {}, dt.gsap = lt, (yt.gsapVersions || (yt.gsapVersions = [])).push(lt.version), pr(He || yt.GreenSockGlobals || !yt.gsap && yt || {}), Tr.forEach(Er)), l = typeof requestAnimationFrame < "u" && requestAnimationFrame, h && g.sleep(), u = l || function(I) {
        return setTimeout(I, a - g.time * 1e3 + 1 | 0);
      }, we = 1, p(2));
    },
    sleep: function() {
      (l ? cancelAnimationFrame : clearTimeout)(h), we = 0, u = Ce;
    },
    lagSmoothing: function(I, w) {
      t = I || 1 / 0, e = Math.min(w || 33, t);
    },
    fps: function(I) {
      n = 1e3 / (I || 240), a = g.time * 1e3 + n;
    },
    add: function(I, w, f) {
      var C = w ? function(d, c, J, m) {
        I(d, c, J, m), g.remove(C);
      } : I;
      return g.remove(I), s[f ? "unshift" : "push"](C), le(), C;
    },
    remove: function(I, w) {
      ~(w = s.indexOf(I)) && s.splice(w, 1) && S >= w && S--;
    },
    _listeners: s
  }, g;
})(), le = function() {
  return !we && At.wake();
}, K = {}, Yn = /^[\d.\-M][\d.\-,\s]/, bn = /["']/g, Mn = function(t) {
  for (var e = {}, i = t.substr(1, t.length - 3).split(":"), r = i[0], n = 1, a = i.length, s, h, u; n < a; n++)
    h = i[n], s = n !== a - 1 ? h.lastIndexOf(",") : h.length, u = h.substr(0, s), e[r] = isNaN(u) ? u.replace(bn, "").trim() : +u, r = h.substr(s + 1).trim();
  return e;
}, zn = function(t) {
  var e = t.indexOf("(") + 1, i = t.indexOf(")"), r = t.indexOf("(", e);
  return t.substring(e, ~r && r < i ? t.indexOf(")", i + 1) : i);
}, Zn = function(t) {
  var e = (t + "").split("("), i = K[e[0]];
  return i && e.length > 1 && i.config ? i.config.apply(null, ~t.indexOf("{") ? [Mn(e[1])] : zn(t).split(",").map(mr)) : K._CE && Yn.test(t) ? K._CE("", t) : i;
}, jn = function(t) {
  return function(e) {
    return 1 - t(1 - e);
  };
}, jt = function(t, e) {
  return t && (M(t) ? t : K[t] || Zn(t)) || e;
}, _t = function(t, e, i, r) {
  i === void 0 && (i = function(h) {
    return 1 - e(1 - h);
  }), r === void 0 && (r = function(h) {
    return h < 0.5 ? e(h * 2) / 2 : 1 - e((1 - h) * 2) / 2;
  });
  var n = {
    easeIn: e,
    easeOut: i,
    easeInOut: r
  }, a;
  return st(t, function(s) {
    K[s] = dt[s] = n, K[a = s.toLowerCase()] = i;
    for (var h in n)
      K[a + (h === "easeIn" ? ".in" : h === "easeOut" ? ".out" : ".inOut")] = K[s + "." + h] = n[h];
  }), n;
}, Nr = function(t) {
  return function(e) {
    return e < 0.5 ? (1 - t(1 - e * 2)) / 2 : 0.5 + t((e - 0.5) * 2) / 2;
  };
}, $e = function o(t, e, i) {
  var r = e >= 1 ? e : 1, n = (i || (t ? 0.3 : 0.45)) / (e < 1 ? e : 1), a = n / si * (Math.asin(1 / r) || 0), s = function(l) {
    return l === 1 ? 1 : r * Math.pow(2, -10 * l) * Cn((l - a) * n) + 1;
  }, h = t === "out" ? s : t === "in" ? function(u) {
    return 1 - s(1 - u);
  } : Nr(s);
  return n = si / n, h.config = function(u, l) {
    return o(t, u, l);
  }, h;
}, ti = function o(t, e) {
  e === void 0 && (e = 1.70158);
  var i = function(a) {
    return a ? --a * a * ((e + 1) * a + e) + 1 : 0;
  }, r = t === "out" ? i : t === "in" ? function(n) {
    return 1 - i(1 - n);
  } : Nr(i);
  return r.config = function(n) {
    return o(t, n);
  }, r;
};
st("Linear,Quad,Cubic,Quart,Quint,Strong", function(o, t) {
  var e = t < 5 ? t + 1 : t;
  _t(o + ",Power" + (e - 1), t ? function(i) {
    return Math.pow(i, e);
  } : function(i) {
    return i;
  }, function(i) {
    return 1 - Math.pow(1 - i, e);
  }, function(i) {
    return i < 0.5 ? Math.pow(i * 2, e) / 2 : 1 - Math.pow((1 - i) * 2, e) / 2;
  });
});
K.Linear.easeNone = K.none = K.Linear.easeIn;
_t("Elastic", $e("in"), $e("out"), $e());
(function(o, t) {
  var e = 1 / t, i = 2 * e, r = 2.5 * e, n = function(s) {
    return s < e ? o * s * s : s < i ? o * Math.pow(s - 1.5 / t, 2) + 0.75 : s < r ? o * (s -= 2.25 / t) * s + 0.9375 : o * Math.pow(s - 2.625 / t, 2) + 0.984375;
  };
  _t("Bounce", function(a) {
    return 1 - n(1 - a);
  }, n);
})(7.5625, 2.75);
_t("Expo", function(o) {
  return Math.pow(2, 10 * (o - 1)) * o + o * o * o * o * o * o * (1 - o);
});
_t("Circ", function(o) {
  return -(Ar(1 - o * o) - 1);
});
_t("Sine", function(o) {
  return o === 1 ? 1 : -In(o * dn) + 1;
});
_t("Back", ti("in"), ti("out"), ti());
K.SteppedEase = K.steps = dt.SteppedEase = {
  config: function(t, e) {
    t === void 0 && (t = 1);
    var i = 1 / t, r = t + (e ? 0 : 1), n = e ? 1 : 0, a = 1 - E;
    return function(s) {
      return ((r * Be(0, a, s) | 0) + n) * i;
    };
  }
};
pe.ease = K["quad.out"];
st("onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt", function(o) {
  return Di += o + "," + o + "Params,";
});
var Gr = function(t, e) {
  this.id = pn++, t._gsap = this, this.target = t, this.harness = e, this.get = e ? e.get : Jr, this.set = e ? e.getSetter : Pi;
}, me = /* @__PURE__ */ (function() {
  function o(e) {
    this.vars = e, this._delay = +e.delay || 0, (this._repeat = e.repeat === 1 / 0 ? -2 : e.repeat || 0) && (this._rDelay = e.repeatDelay || 0, this._yoyo = !!e.yoyo || !!e.yoyoEase), this._ts = 1, he(this, +e.duration, 1, 1), this.data = e.data, R && (this._ctx = R, R.data.push(this)), we || At.wake();
  }
  var t = o.prototype;
  return t.delay = function(i) {
    return i || i === 0 ? (this.parent && this.parent.smoothChildTiming && this.startTime(this._start + i - this._delay), this._delay = i, this) : this._delay;
  }, t.duration = function(i) {
    return arguments.length ? this.totalDuration(this._repeat > 0 ? i + (i + this._rDelay) * this._repeat : i) : this.totalDuration() && this._dur;
  }, t.totalDuration = function(i) {
    return arguments.length ? (this._dirty = 0, he(this, this._repeat < 0 ? i : (i - this._repeat * this._rDelay) / (this._repeat + 1))) : this._tDur;
  }, t.totalTime = function(i, r) {
    if (le(), !arguments.length)
      return this._tTime;
    var n = this._dp;
    if (n && n.smoothChildTiming && this._ts) {
      for (Ye(this, i), !n._dp || n.parent || Br(n, this); n && n.parent; )
        n.parent._time !== n._start + (n._ts >= 0 ? n._tTime / n._ts : (n.totalDuration() - n._tTime) / -n._ts) && n.totalTime(n._tTime, !0), n = n.parent;
      !this.parent && this._dp.autoRemoveChildren && (this._ts > 0 && i < this._tDur || this._ts < 0 && i > 0 || !this._tDur && !i) && Qt(this._dp, this, this._start - this._delay);
    }
    return (this._tTime !== i || !this._dur && !r || this._initted && Math.abs(this._zTime) === E || !this._initted && this._dur && i || !i && !this._initted && (this.add || this._ptLookup)) && (this._ts || (this._pTime = i), wr(this, i, r)), this;
  }, t.time = function(i, r) {
    return arguments.length ? this.totalTime(Math.min(this.totalDuration(), i + zi(this)) % (this._dur + this._rDelay) || (i ? this._dur : 0), r) : this._time;
  }, t.totalProgress = function(i, r) {
    return arguments.length ? this.totalTime(this.totalDuration() * i, r) : this.totalDuration() ? Math.min(1, this._tTime / this._tDur) : this.rawTime() >= 0 && this._initted ? 1 : 0;
  }, t.progress = function(i, r) {
    return arguments.length ? this.totalTime(this.duration() * (this._yoyo && !(this.iteration() & 1) ? 1 - i : i) + zi(this), r) : this.duration() ? Math.min(1, this._time / this._dur) : this.rawTime() > 0 ? 1 : 0;
  }, t.iteration = function(i, r) {
    var n = this.duration() + this._rDelay;
    return arguments.length ? this.totalTime(this._time + (i - 1) * n, r) : this._repeat ? oe(this._tTime, n) + 1 : 1;
  }, t.timeScale = function(i, r) {
    if (!arguments.length)
      return this._rts === -E ? 0 : this._rts;
    if (this._rts === i)
      return this;
    var n = this.parent && this._ts ? Re(this.parent._time, this) : this._tTime;
    return this._rts = +i || 0, this._ts = this._ps || i === -E ? 0 : this._rts, this.totalTime(Be(-Math.abs(this._delay), this.totalDuration(), n), r !== !1), qe(this), On(this);
  }, t.paused = function(i) {
    return arguments.length ? (this._ps !== i && (this._ps = i, i ? (this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()), this._ts = this._act = 0) : (le(), this._ts = this._rts, this.totalTime(this.parent && !this.parent.smoothChildTiming ? this.rawTime() : this._tTime || this._pTime, this.progress() === 1 && Math.abs(this._zTime) !== E && (this._tTime -= E)))), this) : this._ps;
  }, t.startTime = function(i) {
    if (arguments.length) {
      this._start = W(i);
      var r = this.parent || this._dp;
      return r && (r._sort || !this.parent) && Qt(r, this, this._start - this._delay), this;
    }
    return this._start;
  }, t.endTime = function(i) {
    return this._start + (at(i) ? this.totalDuration() : this.duration()) / Math.abs(this._ts || 1);
  }, t.rawTime = function(i) {
    var r = this.parent || this._dp;
    return r ? i && (!this._ts || this._repeat && this._time && this.totalProgress() < 1) ? this._tTime % (this._dur + this._rDelay) : this._ts ? Re(r.rawTime(i), this) : this._tTime : this._tTime;
  }, t.revert = function(i) {
    i === void 0 && (i = Bn);
    var r = X;
    return X = i, xi(this) && (this.timeline && this.timeline.revert(i), this.totalTime(-0.01, i.suppressEvents)), this.data !== "nested" && i.kill !== !1 && this.kill(), X = r, this;
  }, t.globalTime = function(i) {
    for (var r = this, n = arguments.length ? i : r.rawTime(); r; )
      n = r._start + n / (Math.abs(r._ts) || 1), r = r._dp;
    return !this.parent && this._sat ? this._sat.globalTime(i) : n;
  }, t.repeat = function(i) {
    return arguments.length ? (this._repeat = i === 1 / 0 ? -2 : i, Zi(this)) : this._repeat === -2 ? 1 / 0 : this._repeat;
  }, t.repeatDelay = function(i) {
    if (arguments.length) {
      var r = this._time;
      return this._rDelay = i, Zi(this), r ? this.time(r) : this;
    }
    return this._rDelay;
  }, t.yoyo = function(i) {
    return arguments.length ? (this._yoyo = i, this) : this._yoyo;
  }, t.seek = function(i, r) {
    return this.totalTime(Ct(this, i), at(r));
  }, t.restart = function(i, r) {
    return this.play().totalTime(i ? -this._delay : 0, at(r)), this._dur || (this._zTime = -E), this;
  }, t.play = function(i, r) {
    return i != null && this.seek(i, r), this.reversed(!1).paused(!1);
  }, t.reverse = function(i, r) {
    return i != null && this.seek(i || this.totalDuration(), r), this.reversed(!0).paused(!1);
  }, t.pause = function(i, r) {
    return i != null && this.seek(i, r), this.paused(!0);
  }, t.resume = function() {
    return this.paused(!1);
  }, t.reversed = function(i) {
    return arguments.length ? (!!i !== this.reversed() && this.timeScale(-this._rts || (i ? -E : 0)), this) : this._rts < 0;
  }, t.invalidate = function() {
    return this._initted = this._act = 0, this._zTime = -E, this;
  }, t.isActive = function() {
    var i = this.parent || this._dp, r = this._start, n;
    return !!(!i || this._ts && this._initted && i.isActive() && (n = i.rawTime(!0)) >= r && n < this.endTime(!0) - E);
  }, t.eventCallback = function(i, r, n) {
    var a = this.vars;
    return arguments.length > 1 ? (r ? (a[i] = r, n && (a[i + "Params"] = n), i === "onUpdate" && (this._onUpdate = r)) : delete a[i], this) : a[i];
  }, t.then = function(i) {
    var r = this, n = r._prom;
    return new Promise(function(a) {
      var s = M(i) ? i : yr, h = function() {
        var l = r.then;
        r.then = null, n && n(), M(s) && (s = s(r)) && (s.then || s === r) && (r.then = l), a(s), r.then = l;
      };
      r._initted && r.totalProgress() === 1 && r._ts >= 0 || !r._tTime && r._ts < 0 ? h() : r._prom = h;
    });
  }, t.kill = function() {
    ge(this);
  }, o;
})();
pt(me.prototype, {
  _time: 0,
  _start: 0,
  _end: 0,
  _tTime: 0,
  _tDur: 0,
  _dirty: 0,
  _repeat: 0,
  _yoyo: !1,
  parent: null,
  _initted: !1,
  _rDelay: 0,
  _ts: 1,
  _dp: 0,
  ratio: 0,
  _zTime: -E,
  _prom: 0,
  _ps: !1,
  _rts: 1
});
var rt = /* @__PURE__ */ (function(o) {
  gr(t, o);
  function t(i, r) {
    var n;
    return i === void 0 && (i = {}), n = o.call(this, i) || this, n.labels = {}, n.smoothChildTiming = !!i.smoothChildTiming, n.autoRemoveChildren = !!i.autoRemoveChildren, n._sort = at(i.sortChildren), N && Qt(i.parent || N, xt(n), r), i.reversed && n.reverse(), i.paused && n.paused(!0), i.scrollTrigger && Dr(xt(n), i.scrollTrigger), n;
  }
  var e = t.prototype;
  return e.to = function(r, n, a) {
    return Se(0, arguments, this), this;
  }, e.from = function(r, n, a) {
    return Se(1, arguments, this), this;
  }, e.fromTo = function(r, n, a, s) {
    return Se(2, arguments, this), this;
  }, e.set = function(r, n, a) {
    return n.duration = 0, n.parent = this, ce(n).repeatDelay || (n.repeat = 0), n.immediateRender = !!n.immediateRender, new Z(r, n, Ct(this, a), 1), this;
  }, e.call = function(r, n, a) {
    return Qt(this, Z.delayedCall(0, r, n), a);
  }, e.staggerTo = function(r, n, a, s, h, u, l) {
    return a.duration = n, a.stagger = a.stagger || s, a.onComplete = u, a.onCompleteParams = l, a.parent = this, new Z(r, a, Ct(this, h)), this;
  }, e.staggerFrom = function(r, n, a, s, h, u, l) {
    return a.runBackwards = 1, ce(a).immediateRender = at(a.immediateRender), this.staggerTo(r, n, a, s, h, u, l);
  }, e.staggerFromTo = function(r, n, a, s, h, u, l, g) {
    return s.startAt = a, ce(s).immediateRender = at(s.immediateRender), this.staggerTo(r, n, s, h, u, l, g);
  }, e.render = function(r, n, a) {
    var s = this._time, h = this._dirty ? this.totalDuration() : this._tDur, u = this._dur, l = r <= 0 ? 0 : W(r), g = this._zTime < 0 != r < 0 && (this._initted || !u), A, S, p, k, I, w, f, C, d, c, J, m;
    if (this !== N && l > h && r >= 0 && (l = h), l !== this._tTime || a || g) {
      if (s !== this._time && u && (l += this._time - s, r += this._time - s), A = l, d = this._start, C = this._ts, w = !C, g && (u || (s = this._zTime), (r || !n) && (this._zTime = r)), this._repeat) {
        if (J = this._yoyo, I = u + this._rDelay, this._repeat < -1 && r < 0)
          return this.totalTime(I * 100 + r, n, a);
        if (A = W(l % I), l === h ? (k = this._repeat, A = u) : (c = W(l / I), k = ~~c, k && k === c && (A = u, k--), A > u && (A = u)), c = oe(this._tTime, I), !s && this._tTime && c !== k && this._tTime - c * I - this._dur <= 0 && (c = k), J && k & 1 && (A = u - A, m = 1), k !== c && !this._lock) {
          var Q = J && c & 1, y = Q === (J && k & 1);
          if (k < c && (Q = !Q), s = Q ? 0 : l % u ? u : l, this._lock = 1, this.render(s || (m ? 0 : W(k * I)), n, !u)._lock = 0, this._tTime = l, !n && this.parent && kt(this, "onRepeat"), this.vars.repeatRefresh && !m && (this.invalidate()._lock = 1, c = k), s && s !== this._time || w !== !this._ts || this.vars.onRepeat && !this.parent && !this._act)
            return this;
          if (u = this._dur, h = this._tDur, y && (this._lock = 2, s = Q ? u : -1e-4, this.render(s, !0), this.vars.repeatRefresh && !m && this.invalidate()), this._lock = 0, !this._ts && !w)
            return this;
        }
      }
      if (this._hasPause && !this._forcing && this._lock < 2 && (f = Fn(this, W(s), W(A)), f && (l -= A - (A = f._start))), this._tTime = l, this._time = A, this._act = !!C, this._initted || (this._onUpdate = this.vars.onUpdate, this._initted = 1, this._zTime = r, s = 0), !s && l && u && !n && !c && (kt(this, "onStart"), this._tTime !== l))
        return this;
      if (A >= s && r >= 0)
        for (S = this._first; S; ) {
          if (p = S._next, (S._act || A >= S._start) && S._ts && f !== S) {
            if (S.parent !== this)
              return this.render(r, n, a);
            if (S.render(S._ts > 0 ? (A - S._start) * S._ts : (S._dirty ? S.totalDuration() : S._tDur) + (A - S._start) * S._ts, n, a), A !== this._time || !this._ts && !w) {
              f = 0, p && (l += this._zTime = -E);
              break;
            }
          }
          S = p;
        }
      else {
        S = this._last;
        for (var B = r < 0 ? r : A; S; ) {
          if (p = S._prev, (S._act || B <= S._end) && S._ts && f !== S) {
            if (S.parent !== this)
              return this.render(r, n, a);
            if (S.render(S._ts > 0 ? (B - S._start) * S._ts : (S._dirty ? S.totalDuration() : S._tDur) + (B - S._start) * S._ts, n, a || X && xi(S)), A !== this._time || !this._ts && !w) {
              f = 0, p && (l += this._zTime = B ? -E : E);
              break;
            }
          }
          S = p;
        }
      }
      if (f && !n && (this.pause(), f.render(A >= s ? 0 : -E)._zTime = A >= s ? 1 : -1, this._ts))
        return this._start = d, qe(this), this.render(r, n, a);
      this._onUpdate && !n && kt(this, "onUpdate", !0), (l === h && this._tTime >= this.totalDuration() || !l && s) && (d === this._start || Math.abs(C) !== Math.abs(this._ts)) && (this._lock || ((r || !u) && (l === h && this._ts > 0 || !l && this._ts < 0) && Lt(this, 1), !n && !(r < 0 && !s) && (l || s || !h) && (kt(this, l === h && r >= 0 ? "onComplete" : "onReverseComplete", !0), this._prom && !(l < h && this.timeScale() > 0) && this._prom())));
    }
    return this;
  }, e.add = function(r, n) {
    var a = this;
    if (Kt(n) || (n = Ct(this, n, r)), !(r instanceof me)) {
      if ($(r))
        return r.forEach(function(s) {
          return a.add(s, n);
        }), this;
      if (j(r))
        return this.addLabel(r, n);
      if (M(r))
        r = Z.delayedCall(0, r);
      else
        return this;
    }
    return this !== r ? Qt(this, r, n) : this;
  }, e.getChildren = function(r, n, a, s) {
    r === void 0 && (r = !0), n === void 0 && (n = !0), a === void 0 && (a = !0), s === void 0 && (s = -Jt);
    for (var h = [], u = this._first; u; )
      u._start >= s && (u instanceof Z ? n && h.push(u) : (a && h.push(u), r && h.push.apply(h, u.getChildren(!0, n, a)))), u = u._next;
    return h;
  }, e.getById = function(r) {
    for (var n = this.getChildren(1, 1, 1), a = n.length; a--; )
      if (n[a].vars.id === r)
        return n[a];
  }, e.remove = function(r) {
    return j(r) ? this.removeLabel(r) : M(r) ? this.killTweensOf(r) : (r.parent === this && Ve(this, r), r === this._recent && (this._recent = this._last), Zt(this));
  }, e.totalTime = function(r, n) {
    return arguments.length ? (this._forcing = 1, !this._dp && this._ts && (this._start = W(At.time - (this._ts > 0 ? r / this._ts : (this.totalDuration() - r) / -this._ts))), o.prototype.totalTime.call(this, r, n), this._forcing = 0, this) : this._tTime;
  }, e.addLabel = function(r, n) {
    return this.labels[r] = Ct(this, n), this;
  }, e.removeLabel = function(r) {
    return delete this.labels[r], this;
  }, e.addPause = function(r, n, a) {
    var s = Z.delayedCall(0, n || Ce, a);
    return s.data = "isPause", this._hasPause = 1, Qt(this, s, Ct(this, r));
  }, e.removePause = function(r) {
    var n = this._first;
    for (r = Ct(this, r); n; )
      n._start === r && n.data === "isPause" && Lt(n), n = n._next;
  }, e.killTweensOf = function(r, n, a) {
    for (var s = this.getTweensOf(r, a), h = s.length; h--; )
      Ft !== s[h] && s[h].kill(r, n);
    return this;
  }, e.getTweensOf = function(r, n) {
    for (var a = [], s = wt(r), h = this._first, u = Kt(n), l; h; )
      h instanceof Z ? Dn(h._targets, s) && (u ? (!Ft || h._initted && h._ts) && h.globalTime(0) <= n && h.globalTime(h.totalDuration()) > n : !n || h.isActive()) && a.push(h) : (l = h.getTweensOf(s, n)).length && a.push.apply(a, l), h = h._next;
    return a;
  }, e.tweenTo = function(r, n) {
    n = n || {};
    var a = this, s = Ct(a, r), h = n, u = h.startAt, l = h.onStart, g = h.onStartParams, A = h.immediateRender, S, p = Z.to(a, pt({
      ease: n.ease || "none",
      lazy: !1,
      immediateRender: !1,
      time: s,
      overwrite: "auto",
      duration: n.duration || Math.abs((s - (u && "time" in u ? u.time : a._time)) / a.timeScale()) || E,
      onStart: function() {
        if (a.pause(), !S) {
          var I = n.duration || Math.abs((s - (u && "time" in u ? u.time : a._time)) / a.timeScale());
          p._dur !== I && he(p, I, 0, 1).render(p._time, !0, !0), S = 1;
        }
        l && l.apply(p, g || []);
      }
    }, n));
    return A ? p.render(0) : p;
  }, e.tweenFromTo = function(r, n, a) {
    return this.tweenTo(n, pt({
      startAt: {
        time: Ct(this, r)
      }
    }, a));
  }, e.recent = function() {
    return this._recent;
  }, e.nextLabel = function(r) {
    return r === void 0 && (r = this._time), ji(this, Ct(this, r));
  }, e.previousLabel = function(r) {
    return r === void 0 && (r = this._time), ji(this, Ct(this, r), 1);
  }, e.currentLabel = function(r) {
    return arguments.length ? this.seek(r, !0) : this.previousLabel(this._time + E);
  }, e.shiftChildren = function(r, n, a) {
    a === void 0 && (a = 0);
    var s = this._first, h = this.labels, u;
    for (r = W(r); s; )
      s._start >= a && (s._start += r, s._end += r), s = s._next;
    if (n)
      for (u in h)
        h[u] >= a && (h[u] += r);
    return Zt(this);
  }, e.invalidate = function(r) {
    var n = this._first;
    for (this._lock = 0; n; )
      n.invalidate(r), n = n._next;
    return o.prototype.invalidate.call(this, r);
  }, e.clear = function(r) {
    r === void 0 && (r = !0);
    for (var n = this._first, a; n; )
      a = n._next, this.remove(n), n = a;
    return this._dp && (this._time = this._tTime = this._pTime = 0), r && (this.labels = {}), Zt(this);
  }, e.totalDuration = function(r) {
    var n = 0, a = this, s = a._last, h = Jt, u, l, g;
    if (arguments.length)
      return a.timeScale((a._repeat < 0 ? a.duration() : a.totalDuration()) / (a.reversed() ? -r : r));
    if (a._dirty) {
      for (g = a.parent; s; )
        u = s._prev, s._dirty && s.totalDuration(), l = s._start, l > h && a._sort && s._ts && !a._lock ? (a._lock = 1, Qt(a, s, l - s._delay, 1)._lock = 0) : h = l, l < 0 && s._ts && (n -= l, (!g && !a._dp || g && g.smoothChildTiming) && (a._start += W(l / a._ts), a._time -= l, a._tTime -= l), a.shiftChildren(-l, !1, -1 / 0), h = 0), s._end > n && s._ts && (n = s._end), s = u;
      he(a, a === N && a._time > n ? a._time : n, 1, 1), a._dirty = 0;
    }
    return a._tDur;
  }, t.updateRoot = function(r) {
    if (N._ts && (wr(N, Re(r, N)), Cr = At.frame), At.frame >= bi) {
      bi += St.autoSleep || 120;
      var n = N._first;
      if ((!n || !n._ts) && St.autoSleep && At._listeners.length < 2) {
        for (; n && !n._ts; )
          n = n._next;
        n || At.sleep();
      }
    }
  }, t;
})(me);
pt(rt.prototype, {
  _lock: 0,
  _hasPause: 0,
  _forcing: 0
});
var Xn = function(t, e, i, r, n, a, s) {
  var h = new ot(this._pt, t, e, 0, 1, zr, null, n), u = 0, l = 0, g, A, S, p, k, I, w, f;
  for (h.b = i, h.e = r, i += "", r += "", (w = ~r.indexOf("random(")) && (r = Je(r)), a && (f = [i, r], a(f, t, e), i = f[0], r = f[1]), A = i.match(je) || []; g = je.exec(r); )
    p = g[0], k = r.substring(u, g.index), S ? S = (S + 1) % 5 : k.substr(-5) === "rgba(" && (S = 1), p !== A[l++] && (I = parseFloat(A[l - 1]) || 0, h._pt = {
      _next: h._pt,
      p: k || l === 1 ? k : ",",
      //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
      s: I,
      c: p.charAt(1) === "=" ? re(I, p) - I : parseFloat(p) - I,
      m: S && S < 4 ? Math.round : 0
    }, u = je.lastIndex);
  return h.c = u < r.length ? r.substring(u, r.length) : "", h.fp = s, (Sr.test(r) || w) && (h.e = 0), this._pt = h, h;
}, Oi = function(t, e, i, r, n, a, s, h, u, l) {
  M(r) && (r = r(n || 0, t, a));
  var g = t[e], A = i !== "get" ? i : M(g) ? u ? t[e.indexOf("set") || !M(t["get" + e.substr(3)]) ? e : "get" + e.substr(3)](u) : t[e]() : g, S = M(g) ? u ? ia : br : Ui, p;
  if (j(r) && (~r.indexOf("random(") && (r = Je(r)), r.charAt(1) === "=" && (p = re(A, r) + (_(A) || 0), (p || p === 0) && (r = p))), !l || A !== r || ki)
    return !isNaN(A * r) && r !== "" ? (p = new ot(this._pt, t, e, +A || 0, r - (A || 0), typeof g == "boolean" ? na : Mr, 0, S), u && (p.fp = u), s && p.modifier(s, this, t), this._pt = p) : (!g && !(e in t) && Qi(e, r), Xn.call(this, t, e, A, r, S, h || St.stringFilter, u));
}, _n = function(t, e, i, r, n) {
  if (M(t) && (t = de(t, n, e, i, r)), !Dt(t) || t.style && t.nodeType || $(t) || kr(t))
    return j(t) ? de(t, n, e, i, r) : t;
  var a = {}, s;
  for (s in t)
    a[s] = de(t[s], n, e, i, r);
  return a;
}, Vr = function(t, e, i, r, n, a) {
  var s, h, u, l;
  if (gt[t] && (s = new gt[t]()).init(n, s.rawVars ? e[t] : _n(e[t], r, n, a, i), i, r, a) !== !1 && (i._pt = h = new ot(i._pt, n, t, 0, 1, s.render, s, 0, s.priority), i !== ie))
    for (u = i._ptLookup[i._targets.indexOf(n)], l = s._props.length; l--; )
      u[s._props[l]] = h;
  return s;
}, Ft, ki, Ki = function o(t, e, i) {
  var r = t.vars, n = r.ease, a = r.startAt, s = r.immediateRender, h = r.lazy, u = r.onUpdate, l = r.runBackwards, g = r.yoyoEase, A = r.keyframes, S = r.autoRevert, p = t._dur, k = t._startAt, I = t._targets, w = t.parent, f = w && w.data === "nested" ? w.vars.targets : I, C = t._overwrite === "auto" && !Ji, d = t.timeline, c = r.easeReverse || g, J, m, Q, y, B, x, v, O, P, H, U, F, V;
  if (d && (!A || !n) && (n = "none"), t._ease = jt(n, pe.ease), t._rEase = c && (jt(c) || t._ease), t._from = !d && !!r.runBackwards, t._from && (t.ratio = 1), !d || A && !r.stagger) {
    if (O = I[0] ? zt(I[0]).harness : 0, F = O && r[O.prop], J = Ee(r, Bi), k && (k._zTime < 0 && k.progress(1), e < 0 && l && s && !S ? k.render(-1, !0) : k.revert(l && p ? xe : Qn), k._lazy = 0), a) {
      if (Lt(t._startAt = Z.set(I, pt({
        data: "isStart",
        overwrite: !1,
        parent: w,
        immediateRender: !0,
        lazy: !k && at(h),
        startAt: null,
        delay: 0,
        onUpdate: u && function() {
          return kt(t, "onUpdate");
        },
        stagger: 0
      }, a))), t._startAt._dp = 0, t._startAt._sat = t, e < 0 && (X || !s && !S) && t._startAt.revert(xe), s && p && e <= 0 && i <= 0) {
        e && (t._zTime = e);
        return;
      }
    } else if (l && p && !k) {
      if (e && (s = !1), Q = pt({
        overwrite: !1,
        data: "isFromStart",
        //we tag the tween with as "isFromStart" so that if [inside a plugin] we need to only do something at the very END of a tween, we have a way of identifying this tween as merely the one that's setting the beginning values for a "from()" tween. For example, clearProps in CSSPlugin should only get applied at the very END of a tween and without this tag, from(...{height:100, clearProps:"height", delay:1}) would wipe the height at the beginning of the tween and after 1 second, it'd kick back in.
        lazy: s && !k && at(h),
        immediateRender: s,
        //zero-duration tweens render immediately by default, but if we're not specifically instructed to render this tween immediately, we should skip this and merely _init() to record the starting values (rendering them immediately would push them to completion which is wasteful in that case - we'd have to render(-1) immediately after)
        stagger: 0,
        parent: w
        //ensures that nested tweens that had a stagger are handled properly, like gsap.from(".class", {y: gsap.utils.wrap([-100,100]), stagger: 0.5})
      }, J), F && (Q[O.prop] = F), Lt(t._startAt = Z.set(I, Q)), t._startAt._dp = 0, t._startAt._sat = t, e < 0 && (X ? t._startAt.revert(xe) : t._startAt.render(-1, !0)), t._zTime = e, !s)
        o(t._startAt, E, E);
      else if (!e)
        return;
    }
    for (t._pt = t._ptCache = 0, h = p && at(h) || h && !p, m = 0; m < I.length; m++) {
      if (B = I[m], v = B._gsap || vi(I)[m]._gsap, t._ptLookup[m] = H = {}, hi[v.id] && Et.length && Te(), U = f === I ? m : f.indexOf(B), O && (P = new O()).init(B, F || J, t, U, f) !== !1 && (t._pt = y = new ot(t._pt, B, P.name, 0, 1, P.render, P, 0, P.priority), P._props.forEach(function(et) {
        H[et] = y;
      }), P.priority && (x = 1)), !O || F)
        for (Q in J)
          gt[Q] && (P = Vr(Q, J, t, U, B, f)) ? P.priority && (x = 1) : H[Q] = y = Oi.call(t, B, Q, "get", J[Q], U, f, 0, r.stringFilter);
      t._op && t._op[m] && t.kill(B, t._op[m]), C && t._pt && (Ft = t, N.killTweensOf(B, H, t.globalTime(e)), V = !t.parent, Ft = 0), t._pt && h && (hi[v.id] = 1);
    }
    x && Zr(t), t._onInit && t._onInit(t);
  }
  t._onUpdate = u, t._initted = (!t._op || t._pt) && !V, A && e <= 0 && d.render(Jt, !0, !0);
}, $n = function(t, e, i, r, n, a, s, h) {
  var u = (t._pt && t._ptCache || (t._ptCache = {}))[e], l, g, A, S;
  if (!u)
    for (u = t._ptCache[e] = [], A = t._ptLookup, S = t._targets.length; S--; ) {
      if (l = A[S][e], l && l.d && l.d._pt)
        for (l = l.d._pt; l && l.p !== e && l.fp !== e; )
          l = l._next;
      if (!l)
        return ki = 1, t.vars[e] = "+=0", Ki(t, s), ki = 0, h ? Ie(e + " not eligible for reset. Try splitting into individual properties") : 1;
      u.push(l);
    }
  for (S = u.length; S--; )
    g = u[S], l = g._pt || g, l.s = (r || r === 0) && !n ? r : l.s + (r || 0) + a * l.c, l.c = i - l.s, g.e && (g.e = z(i) + _(g.e)), g.b && (g.b = l.s + _(g.b));
}, ta = function(t, e) {
  var i = t[0] ? zt(t[0]).harness : 0, r = i && i.aliases, n, a, s, h;
  if (!r)
    return e;
  n = se({}, e);
  for (a in r)
    if (a in n)
      for (h = r[a].split(","), s = h.length; s--; )
        n[h[s]] = n[a];
  return n;
}, ea = function(t, e, i, r) {
  var n = e.ease || r || "power1.inOut", a, s;
  if ($(e))
    s = i[t] || (i[t] = []), e.forEach(function(h, u) {
      return s.push({
        t: u / (e.length - 1) * 100,
        v: h,
        e: n
      });
    });
  else
    for (a in e)
      s = i[a] || (i[a] = []), a === "ease" || s.push({
        t: parseFloat(t),
        v: e[a],
        e: n
      });
}, de = function(t, e, i, r, n) {
  return M(t) ? t.call(e, i, r, n) : j(t) && ~t.indexOf("random(") ? Je(t) : t;
}, qr = Di + "repeat,repeatDelay,yoyo,repeatRefresh,yoyoEase,easeReverse,autoRevert", Yr = {};
st(qr + ",id,stagger,delay,duration,paused,scrollTrigger", function(o) {
  return Yr[o] = 1;
});
var Z = /* @__PURE__ */ (function(o) {
  gr(t, o);
  function t(i, r, n, a) {
    var s;
    typeof r == "number" && (n.duration = r, r = n, n = null), s = o.call(this, a ? r : ce(r)) || this;
    var h = s.vars, u = h.duration, l = h.delay, g = h.immediateRender, A = h.stagger, S = h.overwrite, p = h.keyframes, k = h.defaults, I = h.scrollTrigger, w = r.parent || N, f = ($(i) || kr(i) ? Kt(i[0]) : "length" in r) ? [i] : wt(i), C, d, c, J, m, Q, y, B;
    if (s._targets = f.length ? vi(f) : Ie("GSAP target " + i + " not found. https://gsap.com", !St.nullTargetWarn) || [], s._ptLookup = [], s._overwrite = S, p || A || ve(u) || ve(l)) {
      r = s.vars;
      var x = r.easeReverse || r.yoyoEase;
      if (C = s.timeline = new rt({
        data: "nested",
        defaults: k || {},
        targets: w && w.data === "nested" ? w.vars.targets : f
      }), C.kill(), C.parent = C._dp = xt(s), C._start = 0, A || ve(u) || ve(l)) {
        if (J = f.length, y = A && Kr(A), Dt(A))
          for (m in A)
            ~qr.indexOf(m) && (B || (B = {}), B[m] = A[m]);
        for (d = 0; d < J; d++)
          c = Ee(r, Yr), c.stagger = 0, x && (c.easeReverse = x), B && se(c, B), Q = f[d], c.duration = +de(u, xt(s), d, Q, f), c.delay = (+de(l, xt(s), d, Q, f) || 0) - s._delay, !A && J === 1 && c.delay && (s._delay = l = c.delay, s._start += l, c.delay = 0), C.to(Q, c, y ? y(d, Q, f) : 0), C._ease = K.none;
        C.duration() ? u = l = 0 : s.timeline = 0;
      } else if (p) {
        ce(pt(C.vars.defaults, {
          ease: "none"
        })), C._ease = jt(p.ease || r.ease || "none");
        var v = 0, O, P, H;
        if ($(p))
          p.forEach(function(U) {
            return C.to(f, U, ">");
          }), C.duration();
        else {
          c = {};
          for (m in p)
            m === "ease" || m === "easeEach" || ea(m, p[m], c, p.easeEach);
          for (m in c)
            for (O = c[m].sort(function(U, F) {
              return U.t - F.t;
            }), v = 0, d = 0; d < O.length; d++)
              P = O[d], H = {
                ease: P.e,
                duration: (P.t - (d ? O[d - 1].t : 0)) / 100 * u
              }, H[m] = P.v, C.to(f, H, v), v += H.duration;
          C.duration() < u && C.to({}, {
            duration: u - C.duration()
          });
        }
      }
      u || s.duration(u = C.duration());
    } else
      s.timeline = 0;
    return S === !0 && !Ji && (Ft = xt(s), N.killTweensOf(f), Ft = 0), Qt(w, xt(s), n), r.reversed && s.reverse(), r.paused && s.paused(!0), (g || !u && !p && s._start === W(w._time) && at(g) && Kn(xt(s)) && w.data !== "nested") && (s._tTime = -E, s.render(Math.max(0, -l) || 0)), I && Dr(xt(s), I), s;
  }
  var e = t.prototype;
  return e.render = function(r, n, a) {
    var s = this._time, h = this._tDur, u = this._dur, l = r < 0, g = r > h - E && !l ? h : r < E ? 0 : r, A, S, p, k, I, w, f, C;
    if (!u)
      Pn(this, r, n, a);
    else if (g !== this._tTime || !r || a || !this._initted && this._tTime || this._startAt && this._zTime < 0 !== l || this._lazy) {
      if (A = g, C = this.timeline, this._repeat) {
        if (k = u + this._rDelay, this._repeat < -1 && l)
          return this.totalTime(k * 100 + r, n, a);
        if (A = W(g % k), g === h ? (p = this._repeat, A = u) : (I = W(g / k), p = ~~I, p && p === I ? (A = u, p--) : A > u && (A = u)), w = this._yoyo && p & 1, w && (A = u - A), I = oe(this._tTime, k), A === s && !a && this._initted && p === I)
          return this._tTime = g, this;
        p !== I && this.vars.repeatRefresh && !w && !this._lock && A !== k && this._initted && (this._lock = a = 1, this.render(W(k * p), !0).invalidate()._lock = 0);
      }
      if (!this._initted) {
        if (vr(this, l ? r : A, a, n, g))
          return this._tTime = 0, this;
        if (s !== this._time && !(a && this.vars.repeatRefresh && p !== I))
          return this;
        if (u !== this._dur)
          return this.render(r, n, a);
      }
      if (this._rEase) {
        var d = A < s;
        if (d !== this._inv) {
          var c = d ? s : u - s;
          this._inv = d, this._from && (this.ratio = 1 - this.ratio), this._invRatio = this.ratio, this._invTime = s, this._invRecip = c ? (d ? -1 : 1) / c : 0, this._invScale = d ? -this.ratio : 1 - this.ratio, this._invEase = d ? this._rEase : this._ease;
        }
        this.ratio = f = this._invRatio + this._invScale * this._invEase((A - this._invTime) * this._invRecip);
      } else
        this.ratio = f = this._ease(A / u);
      if (this._from && (this.ratio = f = 1 - f), this._tTime = g, this._time = A, !this._act && this._ts && (this._act = 1, this._lazy = 0), !s && g && !n && !I && (kt(this, "onStart"), this._tTime !== g))
        return this;
      for (S = this._pt; S; )
        S.r(f, S.d), S = S._next;
      C && C.render(r < 0 ? r : C._dur * C._ease(A / this._dur), n, a) || this._startAt && (this._zTime = r), this._onUpdate && !n && (l && li(this, r, n, a), kt(this, "onUpdate")), this._repeat && p !== I && this.vars.onRepeat && !n && this.parent && kt(this, "onRepeat"), (g === this._tDur || !g) && this._tTime === g && (l && !this._onUpdate && li(this, r, !0, !0), (r || !u) && (g === this._tDur && this._ts > 0 || !g && this._ts < 0) && Lt(this, 1), !n && !(l && !s) && (g || s || w) && (kt(this, g === h ? "onComplete" : "onReverseComplete", !0), this._prom && !(g < h && this.timeScale() > 0) && this._prom()));
    }
    return this;
  }, e.targets = function() {
    return this._targets;
  }, e.invalidate = function(r) {
    return (!r || !this.vars.runBackwards) && (this._startAt = 0), this._pt = this._op = this._onUpdate = this._lazy = this.ratio = 0, this._ptLookup = [], this.timeline && this.timeline.invalidate(r), o.prototype.invalidate.call(this, r);
  }, e.resetTo = function(r, n, a, s, h) {
    we || At.wake(), this._ts || this.play();
    var u = Math.min(this._dur, (this._dp._time - this._start) * this._ts), l;
    return this._initted || Ki(this, u), l = this._ease(u / this._dur), $n(this, r, n, a, s, l, u, h) ? this.resetTo(r, n, a, s, 1) : (Ye(this, 0), this.parent || Qr(this._dp, this, "_first", "_last", this._dp._sort ? "_start" : 0), this.render(0));
  }, e.kill = function(r, n) {
    if (n === void 0 && (n = "all"), !r && (!n || n === "all"))
      return this._lazy = this._pt = 0, this.parent ? ge(this) : this.scrollTrigger && this.scrollTrigger.kill(!!X), this;
    if (this.timeline) {
      var a = this.timeline.totalDuration();
      return this.timeline.killTweensOf(r, n, Ft && Ft.vars.overwrite !== !0)._first || ge(this), this.parent && a !== this.timeline.totalDuration() && he(this, this._dur * this.timeline._tDur / a, 0, 1), this;
    }
    var s = this._targets, h = r ? wt(r) : s, u = this._ptLookup, l = this._pt, g, A, S, p, k, I, w;
    if ((!n || n === "all") && xn(s, h))
      return n === "all" && (this._pt = 0), ge(this);
    for (g = this._op = this._op || [], n !== "all" && (j(n) && (k = {}, st(n, function(f) {
      return k[f] = 1;
    }), n = k), n = ta(s, n)), w = s.length; w--; )
      if (~h.indexOf(s[w])) {
        A = u[w], n === "all" ? (g[w] = n, p = A, S = {}) : (S = g[w] = g[w] || {}, p = n);
        for (k in p)
          I = A && A[k], I && ((!("kill" in I.d) || I.d.kill(k) === !0) && Ve(this, I, "_pt"), delete A[k]), S !== "all" && (S[k] = 1);
      }
    return this._initted && !this._pt && l && ge(this), this;
  }, t.to = function(r, n) {
    return new t(r, n, arguments[2]);
  }, t.from = function(r, n) {
    return Se(1, arguments);
  }, t.delayedCall = function(r, n, a, s) {
    return new t(n, 0, {
      immediateRender: !1,
      lazy: !1,
      overwrite: !1,
      delay: r,
      onComplete: n,
      onReverseComplete: n,
      onCompleteParams: a,
      onReverseCompleteParams: a,
      callbackScope: s
    });
  }, t.fromTo = function(r, n, a) {
    return Se(2, arguments);
  }, t.set = function(r, n) {
    return n.duration = 0, n.repeatDelay || (n.repeat = 0), new t(r, n);
  }, t.killTweensOf = function(r, n, a) {
    return N.killTweensOf(r, n, a);
  }, t;
})(me);
pt(Z.prototype, {
  _targets: [],
  _lazy: 0,
  _startAt: 0,
  _op: 0,
  _onInit: 0
});
st("staggerTo,staggerFrom,staggerFromTo", function(o) {
  Z[o] = function() {
    var t = new rt(), e = ui.call(arguments, 0);
    return e.splice(o === "staggerFromTo" ? 5 : 4, 0, 0), t[o].apply(t, e);
  };
});
var Ui = function(t, e, i) {
  return t[e] = i;
}, br = function(t, e, i) {
  return t[e](i);
}, ia = function(t, e, i, r) {
  return t[e](r.fp, i);
}, ra = function(t, e, i) {
  return t.setAttribute(e, i);
}, Pi = function(t, e) {
  return M(t[e]) ? br : wi(t[e]) && t.setAttribute ? ra : Ui;
}, Mr = function(t, e) {
  return e.set(e.t, e.p, Math.round((e.s + e.c * t) * 1e6) / 1e6, e);
}, na = function(t, e) {
  return e.set(e.t, e.p, !!(e.s + e.c * t), e);
}, zr = function(t, e) {
  var i = e._pt, r = "";
  if (!t && e.b)
    r = e.b;
  else if (t === 1 && e.e)
    r = e.e;
  else {
    for (; i; )
      r = i.p + (i.m ? i.m(i.s + i.c * t) : Math.round((i.s + i.c * t) * 1e4) / 1e4) + r, i = i._next;
    r += e.c;
  }
  e.set(e.t, e.p, r, e);
}, Fi = function(t, e) {
  for (var i = e._pt; i; )
    i.r(t, i.d), i = i._next;
}, aa = function(t, e, i, r) {
  for (var n = this._pt, a; n; )
    a = n._next, n.p === r && n.modifier(t, e, i), n = a;
}, sa = function(t) {
  for (var e = this._pt, i, r; e; )
    r = e._next, e.p === t && !e.op || e.op === t ? Ve(this, e, "_pt") : e.dep || (i = 1), e = r;
  return !i;
}, oa = function(t, e, i, r) {
  r.mSet(t, e, r.m.call(r.tween, i, r.mt), r);
}, Zr = function(t) {
  for (var e = t._pt, i, r, n, a; e; ) {
    for (i = e._next, r = n; r && r.pr > e.pr; )
      r = r._next;
    (e._prev = r ? r._prev : a) ? e._prev._next = e : n = e, (e._next = r) ? r._prev = e : a = e, e = i;
  }
  t._pt = n;
}, ot = /* @__PURE__ */ (function() {
  function o(e, i, r, n, a, s, h, u, l) {
    this.t = i, this.s = n, this.c = a, this.p = r, this.r = s || Mr, this.d = h || this, this.set = u || Ui, this.pr = l || 0, this._next = e, e && (e._prev = this);
  }
  var t = o.prototype;
  return t.modifier = function(i, r, n) {
    this.mSet = this.mSet || this.set, this.set = oa, this.m = i, this.mt = n, this.tween = r;
  }, o;
})();
st(Di + "parent,duration,ease,delay,overwrite,runBackwards,startAt,yoyo,immediateRender,repeat,repeatDelay,data,paused,reversed,lazy,callbackScope,stringFilter,id,yoyoEase,stagger,inherit,repeatRefresh,keyframes,autoRevert,scrollTrigger,easeReverse", function(o) {
  return Bi[o] = 1;
});
dt.TweenMax = dt.TweenLite = Z;
dt.TimelineLite = dt.TimelineMax = rt;
N = new rt({
  sortChildren: !1,
  defaults: pe,
  autoRemoveChildren: !0,
  id: "root",
  smoothChildTiming: !0
});
St.stringFilter = Wr;
var Xt = [], Ke = {}, ha = [], _i = 0, la = 0, ei = function(t) {
  return (Ke[t] || ha).map(function(e) {
    return e();
  });
}, ci = function() {
  var t = Date.now(), e = [];
  t - _i > 2 && (ei("matchMediaInit"), Xt.forEach(function(i) {
    var r = i.queries, n = i.conditions, a, s, h, u;
    for (s in r)
      a = yt.matchMedia(r[s]).matches, a && (h = 1), a !== n[s] && (n[s] = a, u = 1);
    u && (i.revert(), h && e.push(i));
  }), ei("matchMediaRevert"), e.forEach(function(i) {
    return i.onMatch(i, function(r) {
      return i.add(null, r);
    });
  }), _i = t, ei("matchMedia"));
}, jr = /* @__PURE__ */ (function() {
  function o(e, i) {
    this.selector = i && gi(i), this.data = [], this._r = [], this.isReverted = !1, this.id = la++, e && this.add(e);
  }
  var t = o.prototype;
  return t.add = function(i, r, n) {
    M(i) && (n = r, r = i, i = M);
    var a = this, s = function() {
      var u = R, l = a.selector, g;
      return u && u !== a && u.data.push(a), n && (a.selector = gi(n)), R = a, g = r.apply(a, arguments), M(g) && a._r.push(g), R = u, a.selector = l, a.isReverted = !1, g;
    };
    return a.last = s, i === M ? s(a, function(h) {
      return a.add(null, h);
    }) : i ? a[i] = s : s;
  }, t.ignore = function(i) {
    var r = R;
    R = null, i(this), R = r;
  }, t.getTweens = function() {
    var i = [];
    return this.data.forEach(function(r) {
      return r instanceof o ? i.push.apply(i, r.getTweens()) : r instanceof Z && !(r.parent && r.parent.data === "nested") && i.push(r);
    }), i;
  }, t.clear = function() {
    this._r.length = this.data.length = 0;
  }, t.kill = function(i, r) {
    var n = this;
    if (i ? (function() {
      for (var s = n.getTweens(), h = n.data.length, u; h--; )
        u = n.data[h], u.data === "isFlip" && (u.revert(), u.getChildren(!0, !0, !1).forEach(function(l) {
          return s.splice(s.indexOf(l), 1);
        }));
      for (s.map(function(l) {
        return {
          g: l._dur || l._delay || l._sat && !l._sat.vars.immediateRender ? l.globalTime(0) : -1 / 0,
          t: l
        };
      }).sort(function(l, g) {
        return g.g - l.g || -1 / 0;
      }).forEach(function(l) {
        return l.t.revert(i);
      }), h = n.data.length; h--; )
        u = n.data[h], u instanceof rt ? u.data !== "nested" && (u.scrollTrigger && u.scrollTrigger.revert(), u.kill()) : !(u instanceof Z) && u.revert && u.revert(i);
      n._r.forEach(function(l) {
        return l(i, n);
      }), n.isReverted = !0;
    })() : this.data.forEach(function(s) {
      return s.kill && s.kill();
    }), this.clear(), r)
      for (var a = Xt.length; a--; )
        Xt[a].id === this.id && Xt.splice(a, 1);
  }, t.revert = function(i) {
    this.kill(i || {});
  }, o;
})(), fa = /* @__PURE__ */ (function() {
  function o(e) {
    this.contexts = [], this.scope = e, R && R.data.push(this);
  }
  var t = o.prototype;
  return t.add = function(i, r, n) {
    Dt(i) || (i = {
      matches: i
    });
    var a = new jr(0, n || this.scope), s = a.conditions = {}, h, u, l;
    R && !a.selector && (a.selector = R.selector), this.contexts.push(a), r = a.add("onMatch", r), a.queries = i;
    for (u in i)
      u === "all" ? l = 1 : (h = yt.matchMedia(i[u]), h && (Xt.indexOf(a) < 0 && Xt.push(a), (s[u] = h.matches) && (l = 1), h.addListener ? h.addListener(ci) : h.addEventListener("change", ci)));
    return l && r(a, function(g) {
      return a.add(null, g);
    }), this;
  }, t.revert = function(i) {
    this.kill(i || {});
  }, t.kill = function(i) {
    this.contexts.forEach(function(r) {
      return r.kill(i, !0);
    });
  }, o;
})(), Le = {
  registerPlugin: function() {
    for (var t = arguments.length, e = new Array(t), i = 0; i < t; i++)
      e[i] = arguments[i];
    e.forEach(function(r) {
      return Er(r);
    });
  },
  timeline: function(t) {
    return new rt(t);
  },
  getTweensOf: function(t, e) {
    return N.getTweensOf(t, e);
  },
  getProperty: function(t, e, i, r) {
    j(t) && (t = wt(t)[0]);
    var n = zt(t || {}).get, a = i ? yr : mr;
    return i === "native" && (i = ""), t && (e ? a((gt[e] && gt[e].get || n)(t, e, i, r)) : function(s, h, u) {
      return a((gt[s] && gt[s].get || n)(t, s, h, u));
    });
  },
  quickSetter: function(t, e, i) {
    if (t = wt(t), t.length > 1) {
      var r = t.map(function(l) {
        return lt.quickSetter(l, e, i);
      }), n = r.length;
      return function(l) {
        for (var g = n; g--; )
          r[g](l);
      };
    }
    t = t[0] || {};
    var a = gt[e], s = zt(t), h = s.harness && (s.harness.aliases || {})[e] || e, u = a ? function(l) {
      var g = new a();
      ie._pt = 0, g.init(t, i ? l + i : l, ie, 0, [t]), g.render(1, g), ie._pt && Fi(1, ie);
    } : s.set(t, h);
    return a ? u : function(l) {
      return u(t, h, i ? l + i : l, s, 1);
    };
  },
  quickTo: function(t, e, i) {
    var r, n = lt.to(t, pt((r = {}, r[e] = "+=0.1", r.paused = !0, r.stagger = 0, r), i || {})), a = function(h, u, l) {
      return n.resetTo(e, h, u, l);
    };
    return a.tween = n, a;
  },
  isTweening: function(t) {
    return N.getTweensOf(t, !0).length > 0;
  },
  defaults: function(t) {
    return t && t.ease && (t.ease = jt(t.ease, pe.ease)), Mi(pe, t || {});
  },
  config: function(t) {
    return Mi(St, t || {});
  },
  registerEffect: function(t) {
    var e = t.name, i = t.effect, r = t.plugins, n = t.defaults, a = t.extendTimeline;
    (r || "").split(",").forEach(function(s) {
      return s && !gt[s] && !dt[s] && Ie(e + " effect requires " + s + " plugin.");
    }), Xe[e] = function(s, h, u) {
      return i(wt(s), pt(h || {}, n), u);
    }, a && (rt.prototype[e] = function(s, h, u) {
      return this.add(Xe[e](s, Dt(h) ? h : (u = h) && {}, this), u);
    });
  },
  registerEase: function(t, e) {
    K[t] = jt(e);
  },
  parseEase: function(t, e) {
    return arguments.length ? jt(t, e) : K;
  },
  getById: function(t) {
    return N.getById(t);
  },
  exportRoot: function(t, e) {
    t === void 0 && (t = {});
    var i = new rt(t), r, n;
    for (i.smoothChildTiming = at(t.smoothChildTiming), N.remove(i), i._dp = 0, i._time = i._tTime = N._time, r = N._first; r; )
      n = r._next, (e || !(!r._dur && r instanceof Z && r.vars.onComplete === r._targets[0])) && Qt(i, r, r._start - r._delay), r = n;
    return Qt(N, i, 0), i;
  },
  context: function(t, e) {
    return t ? new jr(t, e) : R;
  },
  matchMedia: function(t) {
    return new fa(t);
  },
  matchMediaRefresh: function() {
    return Xt.forEach(function(t) {
      var e = t.conditions, i, r;
      for (r in e)
        e[r] && (e[r] = !1, i = 1);
      i && t.revert();
    }) || ci();
  },
  addEventListener: function(t, e) {
    var i = Ke[t] || (Ke[t] = []);
    ~i.indexOf(e) || i.push(e);
  },
  removeEventListener: function(t, e) {
    var i = Ke[t], r = i && i.indexOf(e);
    r >= 0 && i.splice(r, 1);
  },
  utils: {
    wrap: Nn,
    wrapYoyo: Gn,
    distribute: Kr,
    random: Pr,
    snap: Ur,
    normalize: Wn,
    getUnit: _,
    clamp: Tn,
    splitColor: Rr,
    toArray: wt,
    selector: gi,
    mapRange: Hr,
    pipe: Rn,
    unitize: Ln,
    interpolate: Vn,
    shuffle: Or
  },
  install: pr,
  effects: Xe,
  ticker: At,
  updateRoot: rt.updateRoot,
  plugins: gt,
  globalTimeline: N,
  core: {
    PropTween: ot,
    globals: Ir,
    Tween: Z,
    Timeline: rt,
    Animation: me,
    getCache: zt,
    _removeLinkedListItem: Ve,
    reverting: function() {
      return X;
    },
    context: function(t) {
      return t && R && (R.data.push(t), t._ctx = R), R;
    },
    suppressOverwrites: function(t) {
      return Ji = t;
    }
  }
};
st("to,from,fromTo,delayedCall,set,killTweensOf", function(o) {
  return Le[o] = Z[o];
});
At.add(rt.updateRoot);
ie = Le.to({}, {
  duration: 0
});
var ua = function(t, e) {
  for (var i = t._pt; i && i.p !== e && i.op !== e && i.fp !== e; )
    i = i._next;
  return i;
}, ga = function(t, e) {
  var i = t._targets, r, n, a;
  for (r in e)
    for (n = i.length; n--; )
      a = t._ptLookup[n][r], a && (a = a.d) && (a._pt && (a = ua(a, r)), a && a.modifier && a.modifier(e[r], t, i[n], r));
}, ii = function(t, e) {
  return {
    name: t,
    headless: 1,
    rawVars: 1,
    //don't pre-process function-based values or "random()" strings.
    init: function(r, n, a) {
      a._onInit = function(s) {
        var h, u;
        if (j(n) && (h = {}, st(n, function(l) {
          return h[l] = 1;
        }), n = h), e) {
          h = {};
          for (u in n)
            h[u] = e(n[u]);
          n = h;
        }
        ga(s, n);
      };
    }
  };
}, lt = Le.registerPlugin({
  name: "attr",
  init: function(t, e, i, r, n) {
    var a, s, h;
    this.tween = i;
    for (a in e)
      h = t.getAttribute(a) || "", s = this.add(t, "setAttribute", (h || 0) + "", e[a], r, n, 0, 0, a), s.op = a, s.b = h, this._props.push(a);
  },
  render: function(t, e) {
    for (var i = e._pt; i; )
      X ? i.set(i.t, i.p, i.b, i) : i.r(t, i.d), i = i._next;
  }
}, {
  name: "endArray",
  headless: 1,
  init: function(t, e) {
    for (var i = e.length; i--; )
      this.add(t, i, t[i] || 0, e[i], 0, 0, 0, 0, 0, 1);
  }
}, ii("roundProps", Ai), ii("modifiers"), ii("snap", Ur)) || Le;
Z.version = rt.version = lt.version = "3.15.0";
dr = 1;
mi() && le();
K.Power0;
var Aa = K.Power1;
K.Power2;
K.Power3;
K.Power4;
K.Linear;
K.Quad;
K.Cubic;
K.Quart;
K.Quint;
K.Strong;
K.Elastic;
K.Back;
K.SteppedEase;
K.Bounce;
K.Sine;
K.Expo;
K.Circ;
/*!
 * CSSPlugin 3.15.0
 * https://gsap.com
 *
 * Copyright 2008-2026, GreenSock. All rights reserved.
 * Subject to the terms at https://gsap.com/standard-license
 * @author: Jack Doyle, jack@greensock.com
*/
var $i, Ht, ne, Hi, Mt, tr, Ti, ka = function() {
  return typeof window < "u";
}, Ut = {}, bt = 180 / Math.PI, ae = Math.PI / 180, te = Math.atan2, er = 1e8, Ei = /([A-Z])/g, ca = /(left|right|width|margin|padding|x)/i, Sa = /[\s,\(]\S/, Bt = {
  autoAlpha: "opacity,visibility",
  scale: "scaleX,scaleY",
  alpha: "opacity"
}, Si = function(t, e) {
  return e.set(e.t, e.p, Math.round((e.s + e.c * t) * 1e4) / 1e4 + e.u, e);
}, da = function(t, e) {
  return e.set(e.t, e.p, t === 1 ? e.e : Math.round((e.s + e.c * t) * 1e4) / 1e4 + e.u, e);
}, pa = function(t, e) {
  return e.set(e.t, e.p, t ? Math.round((e.s + e.c * t) * 1e4) / 1e4 + e.u : e.b, e);
}, Ia = function(t, e) {
  return e.set(e.t, e.p, t === 1 ? e.e : t ? Math.round((e.s + e.c * t) * 1e4) / 1e4 + e.u : e.b, e);
}, Ca = function(t, e) {
  var i = e.s + e.c * t;
  e.set(e.t, e.p, ~~(i + (i < 0 ? -0.5 : 0.5)) + e.u, e);
}, Xr = function(t, e) {
  return e.set(e.t, e.p, t ? e.e : e.b, e);
}, _r = function(t, e) {
  return e.set(e.t, e.p, t !== 1 ? e.b : e.e, e);
}, Ja = function(t, e, i) {
  return t.style[e] = i;
}, wa = function(t, e, i) {
  return t.style.setProperty(e, i);
}, ma = function(t, e, i) {
  return t._gsap[e] = i;
}, ya = function(t, e, i) {
  return t._gsap.scaleX = t._gsap.scaleY = i;
}, Qa = function(t, e, i, r, n) {
  var a = t._gsap;
  a.scaleX = a.scaleY = i, a.renderTransform(n, a);
}, Ba = function(t, e, i, r, n) {
  var a = t._gsap;
  a[e] = i, a.renderTransform(n, a);
}, G = "transform", ht = G + "Origin", Da = function o(t, e) {
  var i = this, r = this.target, n = r.style, a = r._gsap;
  if (t in Ut && n) {
    if (this.tfm = this.tfm || {}, t !== "transform")
      t = Bt[t] || t, ~t.indexOf(",") ? t.split(",").forEach(function(s) {
        return i.tfm[s] = Ot(r, s);
      }) : this.tfm[t] = a.x ? a[t] : Ot(r, t), t === ht && (this.tfm.zOrigin = a.zOrigin);
    else
      return Bt.transform.split(",").forEach(function(s) {
        return o.call(i, s, e);
      });
    if (this.props.indexOf(G) >= 0)
      return;
    a.svg && (this.svgo = r.getAttribute("data-svg-origin"), this.props.push(ht, e, "")), t = G;
  }
  (n || e) && this.props.push(t, e, n[t]);
}, $r = function(t) {
  t.translate && (t.removeProperty("translate"), t.removeProperty("scale"), t.removeProperty("rotate"));
}, va = function() {
  var t = this.props, e = this.target, i = e.style, r = e._gsap, n, a;
  for (n = 0; n < t.length; n += 3)
    t[n + 1] ? t[n + 1] === 2 ? e[t[n]](t[n + 2]) : e[t[n]] = t[n + 2] : t[n + 2] ? i[t[n]] = t[n + 2] : i.removeProperty(t[n].substr(0, 2) === "--" ? t[n] : t[n].replace(Ei, "-$1").toLowerCase());
  if (this.tfm) {
    for (a in this.tfm)
      r[a] = this.tfm[a];
    r.svg && (r.renderTransform(), e.setAttribute("data-svg-origin", this.svgo || "")), n = Ti(), (!n || !n.isStart) && !i[G] && ($r(i), r.zOrigin && i[ht] && (i[ht] += " " + r.zOrigin + "px", r.zOrigin = 0, r.renderTransform()), r.uncache = 1);
  }
}, tn = function(t, e) {
  var i = {
    target: t,
    props: [],
    revert: va,
    save: Da
  };
  return t._gsap || lt.core.getCache(t), e && t.style && t.nodeType && e.split(",").forEach(function(r) {
    return i.save(r);
  }), i;
}, en, di = function(t, e) {
  var i = Ht.createElementNS ? Ht.createElementNS((e || "http://www.w3.org/1999/xhtml").replace(/^https/, "http"), t) : Ht.createElement(t);
  return i && i.style ? i : Ht.createElement(t);
}, ct = function o(t, e, i) {
  var r = getComputedStyle(t);
  return r[e] || r.getPropertyValue(e.replace(Ei, "-$1").toLowerCase()) || r.getPropertyValue(e) || !i && o(t, fe(e) || e, 1) || "";
}, ir = "O,Moz,ms,Ms,Webkit".split(","), fe = function(t, e, i) {
  var r = e || Mt, n = r.style, a = 5;
  if (t in n && !i)
    return t;
  for (t = t.charAt(0).toUpperCase() + t.substr(1); a-- && !(ir[a] + t in n); )
    ;
  return a < 0 ? null : (a === 3 ? "ms" : a >= 0 ? ir[a] : "") + t;
}, pi = function() {
  ka() && window.document && ($i = window, Ht = $i.document, ne = Ht.documentElement, Mt = di("div") || {
    style: {}
  }, di("div"), G = fe(G), ht = G + "Origin", Mt.style.cssText = "border-width:0;line-height:0;position:absolute;padding:0", en = !!fe("perspective"), Ti = lt.core.reverting, Hi = 1);
}, rr = function(t) {
  var e = t.ownerSVGElement, i = di("svg", e && e.getAttribute("xmlns") || "http://www.w3.org/2000/svg"), r = t.cloneNode(!0), n;
  r.style.display = "block", i.appendChild(r), ne.appendChild(i);
  try {
    n = r.getBBox();
  } catch {
  }
  return i.removeChild(r), ne.removeChild(i), n;
}, nr = function(t, e) {
  for (var i = e.length; i--; )
    if (t.hasAttribute(e[i]))
      return t.getAttribute(e[i]);
}, rn = function(t) {
  var e, i;
  try {
    e = t.getBBox();
  } catch {
    e = rr(t), i = 1;
  }
  return e && (e.width || e.height) || i || (e = rr(t)), e && !e.width && !e.x && !e.y ? {
    x: +nr(t, ["x", "cx", "x1"]) || 0,
    y: +nr(t, ["y", "cy", "y1"]) || 0,
    width: 0,
    height: 0
  } : e;
}, nn = function(t) {
  return !!(t.getCTM && (!t.parentNode || t.ownerSVGElement) && rn(t));
}, Wt = function(t, e) {
  if (e) {
    var i = t.style, r;
    e in Ut && e !== ht && (e = G), i.removeProperty ? (r = e.substr(0, 2), (r === "ms" || e.substr(0, 6) === "webkit") && (e = "-" + e), i.removeProperty(r === "--" ? e : e.replace(Ei, "-$1").toLowerCase())) : i.removeAttribute(e);
  }
}, Tt = function(t, e, i, r, n, a) {
  var s = new ot(t._pt, e, i, 0, 1, a ? _r : Xr);
  return t._pt = s, s.b = r, s.e = n, t._props.push(i), s;
}, ar = {
  deg: 1,
  rad: 1,
  turn: 1
}, xa = {
  grid: 1,
  flex: 1
}, Nt = function o(t, e, i, r) {
  var n = parseFloat(i) || 0, a = (i + "").trim().substr((n + "").length) || "px", s = Mt.style, h = ca.test(e), u = t.tagName.toLowerCase() === "svg", l = (u ? "client" : "offset") + (h ? "Width" : "Height"), g = 100, A = r === "px", S = r === "%", p, k, I, w;
  if (r === a || !n || ar[r] || ar[a])
    return n;
  if (a !== "px" && !A && (n = o(t, e, i, "px")), w = t.getCTM && nn(t), (S || a === "%") && (Ut[e] || ~e.indexOf("adius")))
    return p = w ? t.getBBox()[h ? "width" : "height"] : t[l], z(S ? n / p * g : n / 100 * p);
  if (s[h ? "width" : "height"] = g + (A ? a : r), k = r !== "rem" && ~e.indexOf("adius") || r === "em" && t.appendChild && !u ? t : t.parentNode, w && (k = (t.ownerSVGElement || {}).parentNode), (!k || k === Ht || !k.appendChild) && (k = Ht.body), I = k._gsap, I && S && I.width && h && I.time === At.time && !I.uncache)
    return z(n / I.width * g);
  if (S && (e === "height" || e === "width")) {
    var f = t.style[e];
    t.style[e] = g + r, p = t[l], f ? t.style[e] = f : Wt(t, e);
  } else
    (S || a === "%") && !xa[ct(k, "display")] && (s.position = ct(t, "position")), k === t && (s.position = "static"), k.appendChild(Mt), p = Mt[l], k.removeChild(Mt), s.position = "absolute";
  return h && S && (I = zt(k), I.time = At.time, I.width = k[l]), z(A ? p * n / g : p && n ? g / p * n : 0);
}, Ot = function(t, e, i, r) {
  var n;
  return Hi || pi(), e in Bt && e !== "transform" && (e = Bt[e], ~e.indexOf(",") && (e = e.split(",")[0])), Ut[e] && e !== "transform" ? (n = Qe(t, r), n = e !== "transformOrigin" ? n[e] : n.svg ? n.origin : Ne(ct(t, ht)) + " " + n.zOrigin + "px") : (n = t.style[e], (!n || n === "auto" || r || ~(n + "").indexOf("calc(")) && (n = We[e] && We[e](t, e, i) || ct(t, e) || Jr(t, e) || (e === "opacity" ? 1 : 0))), i && !~(n + "").trim().indexOf(" ") ? Nt(t, e, n, i) + i : n;
}, Oa = function(t, e, i, r) {
  if (!i || i === "none") {
    var n = fe(e, t, 1), a = n && ct(t, n, 1);
    a && a !== i ? (e = n, i = a) : e === "borderColor" && (i = ct(t, "borderTopColor"));
  }
  var s = new ot(this._pt, t.style, e, 0, 1, zr), h = 0, u = 0, l, g, A, S, p, k, I, w, f, C, d, c;
  if (s.b = i, s.e = r, i += "", r += "", r.substring(0, 6) === "var(--" && (r = ct(t, r.substring(4, r.indexOf(")")))), r === "auto" && (k = t.style[e], t.style[e] = r, r = ct(t, e) || r, k ? t.style[e] = k : Wt(t, e)), l = [i, r], Wr(l), i = l[0], r = l[1], A = i.match(ee) || [], c = r.match(ee) || [], c.length) {
    for (; g = ee.exec(r); )
      I = g[0], f = r.substring(h, g.index), p ? p = (p + 1) % 5 : (f.substr(-5) === "rgba(" || f.substr(-5) === "hsla(") && (p = 1), I !== (k = A[u++] || "") && (S = parseFloat(k) || 0, d = k.substr((S + "").length), I.charAt(1) === "=" && (I = re(S, I) + d), w = parseFloat(I), C = I.substr((w + "").length), h = ee.lastIndex - C.length, C || (C = C || St.units[e] || d, h === r.length && (r += C, s.e += C)), d !== C && (S = Nt(t, e, k, C) || 0), s._pt = {
        _next: s._pt,
        p: f || u === 1 ? f : ",",
        //note: SVG spec allows omission of comma/space when a negative sign is wedged between two numbers, like 2.5-5.3 instead of 2.5,-5.3 but when tweening, the negative value may switch to positive, so we insert the comma just in case.
        s: S,
        c: w - S,
        m: p && p < 4 || e === "zIndex" ? Math.round : 0
      });
    s.c = h < r.length ? r.substring(h, r.length) : "";
  } else
    s.r = e === "display" && r === "none" ? _r : Xr;
  return Sr.test(r) && (s.e = 0), this._pt = s, s;
}, sr = {
  top: "0%",
  bottom: "100%",
  left: "0%",
  right: "100%",
  center: "50%"
}, Ka = function(t) {
  var e = t.split(" "), i = e[0], r = e[1] || "50%";
  return (i === "top" || i === "bottom" || r === "left" || r === "right") && (t = i, i = r, r = t), e[0] = sr[i] || i, e[1] = sr[r] || r, e.join(" ");
}, Ua = function(t, e) {
  if (e.tween && e.tween._time === e.tween._dur) {
    var i = e.t, r = i.style, n = e.u, a = i._gsap, s, h, u;
    if (n === "all" || n === !0)
      r.cssText = "", h = 1;
    else
      for (n = n.split(","), u = n.length; --u > -1; )
        s = n[u], Ut[s] && (h = 1, s = s === "transformOrigin" ? ht : G), Wt(i, s);
    h && (Wt(i, G), a && (a.svg && i.removeAttribute("transform"), r.scale = r.rotate = r.translate = "none", Qe(i, 1), a.uncache = 1, $r(r)));
  }
}, We = {
  clearProps: function(t, e, i, r, n) {
    if (n.data !== "isFromStart") {
      var a = t._pt = new ot(t._pt, e, i, 0, 0, Ua);
      return a.u = r, a.pr = -10, a.tween = n, t._props.push(i), 1;
    }
  }
  /* className feature (about 0.4kb gzipped).
  , className(plugin, target, property, endValue, tween) {
  	let _renderClassName = (ratio, data) => {
  			data.css.render(ratio, data.css);
  			if (!ratio || ratio === 1) {
  				let inline = data.rmv,
  					target = data.t,
  					p;
  				target.setAttribute("class", ratio ? data.e : data.b);
  				for (p in inline) {
  					_removeProperty(target, p);
  				}
  			}
  		},
  		_getAllStyles = (target) => {
  			let styles = {},
  				computed = getComputedStyle(target),
  				p;
  			for (p in computed) {
  				if (isNaN(p) && p !== "cssText" && p !== "length") {
  					styles[p] = computed[p];
  				}
  			}
  			_setDefaults(styles, _parseTransform(target, 1));
  			return styles;
  		},
  		startClassList = target.getAttribute("class"),
  		style = target.style,
  		cssText = style.cssText,
  		cache = target._gsap,
  		classPT = cache.classPT,
  		inlineToRemoveAtEnd = {},
  		data = {t:target, plugin:plugin, rmv:inlineToRemoveAtEnd, b:startClassList, e:(endValue.charAt(1) !== "=") ? endValue : startClassList.replace(new RegExp("(?:\\s|^)" + endValue.substr(2) + "(?![\\w-])"), "") + ((endValue.charAt(0) === "+") ? " " + endValue.substr(2) : "")},
  		changingVars = {},
  		startVars = _getAllStyles(target),
  		transformRelated = /(transform|perspective)/i,
  		endVars, p;
  	if (classPT) {
  		classPT.r(1, classPT.d);
  		_removeLinkedListItem(classPT.d.plugin, classPT, "_pt");
  	}
  	target.setAttribute("class", data.e);
  	endVars = _getAllStyles(target, true);
  	target.setAttribute("class", startClassList);
  	for (p in endVars) {
  		if (endVars[p] !== startVars[p] && !transformRelated.test(p)) {
  			changingVars[p] = endVars[p];
  			if (!style[p] && style[p] !== "0") {
  				inlineToRemoveAtEnd[p] = 1;
  			}
  		}
  	}
  	cache.classPT = plugin._pt = new PropTween(plugin._pt, target, "className", 0, 0, _renderClassName, data, 0, -11);
  	if (style.cssText !== cssText) { //only apply if things change. Otherwise, in cases like a background-image that's pulled dynamically, it could cause a refresh. See https://gsap.com/forums/topic/20368-possible-gsap-bug-switching-classnames-in-chrome/.
  		style.cssText = cssText; //we recorded cssText before we swapped classes and ran _getAllStyles() because in cases when a className tween is overwritten, we remove all the related tweening properties from that class change (otherwise class-specific stuff can't override properties we've directly set on the target's style object due to specificity).
  	}
  	_parseTransform(target, true); //to clear the caching of transforms
  	data.css = new gsap.plugins.css();
  	data.css.init(target, changingVars, tween);
  	plugin._props.push(...data.css._props);
  	return 1;
  }
  */
}, ye = [1, 0, 0, 1, 0, 0], an = {}, sn = function(t) {
  return t === "matrix(1, 0, 0, 1, 0, 0)" || t === "none" || !t;
}, or = function(t) {
  var e = ct(t, G);
  return sn(e) ? ye : e.substr(7).match(cr).map(z);
}, Ri = function(t, e) {
  var i = t._gsap || zt(t), r = t.style, n = or(t), a, s, h, u;
  return i.svg && t.getAttribute("transform") ? (h = t.transform.baseVal.consolidate().matrix, n = [h.a, h.b, h.c, h.d, h.e, h.f], n.join(",") === "1,0,0,1,0,0" ? ye : n) : (n === ye && !t.offsetParent && t !== ne && !i.svg && (h = r.display, r.display = "block", a = t.parentNode, (!a || !t.offsetParent && !t.getBoundingClientRect().width) && (u = 1, s = t.nextElementSibling, ne.appendChild(t)), n = or(t), h ? r.display = h : Wt(t, "display"), u && (s ? a.insertBefore(t, s) : a ? a.appendChild(t) : ne.removeChild(t))), e && n.length > 6 ? [n[0], n[1], n[4], n[5], n[12], n[13]] : n);
}, Ii = function(t, e, i, r, n, a) {
  var s = t._gsap, h = n || Ri(t, !0), u = s.xOrigin || 0, l = s.yOrigin || 0, g = s.xOffset || 0, A = s.yOffset || 0, S = h[0], p = h[1], k = h[2], I = h[3], w = h[4], f = h[5], C = e.split(" "), d = parseFloat(C[0]) || 0, c = parseFloat(C[1]) || 0, J, m, Q, y;
  i ? h !== ye && (m = S * I - p * k) && (Q = d * (I / m) + c * (-k / m) + (k * f - I * w) / m, y = d * (-p / m) + c * (S / m) - (S * f - p * w) / m, d = Q, c = y) : (J = rn(t), d = J.x + (~C[0].indexOf("%") ? d / 100 * J.width : d), c = J.y + (~(C[1] || C[0]).indexOf("%") ? c / 100 * J.height : c)), r || r !== !1 && s.smooth ? (w = d - u, f = c - l, s.xOffset = g + (w * S + f * k) - w, s.yOffset = A + (w * p + f * I) - f) : s.xOffset = s.yOffset = 0, s.xOrigin = d, s.yOrigin = c, s.smooth = !!r, s.origin = e, s.originIsAbsolute = !!i, t.style[ht] = "0px 0px", a && (Tt(a, s, "xOrigin", u, d), Tt(a, s, "yOrigin", l, c), Tt(a, s, "xOffset", g, s.xOffset), Tt(a, s, "yOffset", A, s.yOffset)), t.setAttribute("data-svg-origin", d + " " + c);
}, Qe = function(t, e) {
  var i = t._gsap || new Gr(t);
  if ("x" in i && !e && !i.uncache)
    return i;
  var r = t.style, n = i.scaleX < 0, a = "px", s = "deg", h = getComputedStyle(t), u = ct(t, ht) || "0", l, g, A, S, p, k, I, w, f, C, d, c, J, m, Q, y, B, x, v, O, P, H, U, F, V, et, mt, ft, tt, ut, it, It;
  return l = g = A = k = I = w = f = C = d = 0, S = p = 1, i.svg = !!(t.getCTM && nn(t)), h.translate && ((h.translate !== "none" || h.scale !== "none" || h.rotate !== "none") && (r[G] = (h.translate !== "none" ? "translate3d(" + (h.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") + (h.rotate !== "none" ? "rotate(" + h.rotate + ") " : "") + (h.scale !== "none" ? "scale(" + h.scale.split(" ").join(",") + ") " : "") + (h[G] !== "none" ? h[G] : "")), r.scale = r.rotate = r.translate = "none"), m = Ri(t, i.svg), i.svg && (i.uncache ? (V = t.getBBox(), u = i.xOrigin - V.x + "px " + (i.yOrigin - V.y) + "px", F = "") : F = !e && t.getAttribute("data-svg-origin"), Ii(t, F || u, !!F || i.originIsAbsolute, i.smooth !== !1, m)), c = i.xOrigin || 0, J = i.yOrigin || 0, m !== ye && (x = m[0], v = m[1], O = m[2], P = m[3], l = H = m[4], g = U = m[5], m.length === 6 ? (S = Math.sqrt(x * x + v * v), p = Math.sqrt(P * P + O * O), k = x || v ? te(v, x) * bt : 0, f = O || P ? te(O, P) * bt + k : 0, f && (p *= Math.abs(Math.cos(f * ae))), i.svg && (l -= c - (c * x + J * O), g -= J - (c * v + J * P))) : (It = m[6], ut = m[7], mt = m[8], ft = m[9], tt = m[10], it = m[11], l = m[12], g = m[13], A = m[14], Q = te(It, tt), I = Q * bt, Q && (y = Math.cos(-Q), B = Math.sin(-Q), F = H * y + mt * B, V = U * y + ft * B, et = It * y + tt * B, mt = H * -B + mt * y, ft = U * -B + ft * y, tt = It * -B + tt * y, it = ut * -B + it * y, H = F, U = V, It = et), Q = te(-O, tt), w = Q * bt, Q && (y = Math.cos(-Q), B = Math.sin(-Q), F = x * y - mt * B, V = v * y - ft * B, et = O * y - tt * B, it = P * B + it * y, x = F, v = V, O = et), Q = te(v, x), k = Q * bt, Q && (y = Math.cos(Q), B = Math.sin(Q), F = x * y + v * B, V = H * y + U * B, v = v * y - x * B, U = U * y - H * B, x = F, H = V), I && Math.abs(I) + Math.abs(k) > 359.9 && (I = k = 0, w = 180 - w), S = z(Math.sqrt(x * x + v * v + O * O)), p = z(Math.sqrt(U * U + It * It)), Q = te(H, U), f = Math.abs(Q) > 2e-4 ? Q * bt : 0, d = it ? 1 / (it < 0 ? -it : it) : 0), i.svg && (F = t.getAttribute("transform"), i.forceCSS = t.setAttribute("transform", "") || !sn(ct(t, G)), F && t.setAttribute("transform", F))), Math.abs(f) > 90 && Math.abs(f) < 270 && (n ? (S *= -1, f += k <= 0 ? 180 : -180, k += k <= 0 ? 180 : -180) : (p *= -1, f += f <= 0 ? 180 : -180)), e = e || i.uncache, i.x = l - ((i.xPercent = l && (!e && i.xPercent || (Math.round(t.offsetWidth / 2) === Math.round(-l) ? -50 : 0))) ? t.offsetWidth * i.xPercent / 100 : 0) + a, i.y = g - ((i.yPercent = g && (!e && i.yPercent || (Math.round(t.offsetHeight / 2) === Math.round(-g) ? -50 : 0))) ? t.offsetHeight * i.yPercent / 100 : 0) + a, i.z = A + a, i.scaleX = z(S), i.scaleY = z(p), i.rotation = z(k) + s, i.rotationX = z(I) + s, i.rotationY = z(w) + s, i.skewX = f + s, i.skewY = C + s, i.transformPerspective = d + a, (i.zOrigin = parseFloat(u.split(" ")[2]) || !e && i.zOrigin || 0) && (r[ht] = Ne(u)), i.xOffset = i.yOffset = 0, i.force3D = St.force3D, i.renderTransform = i.svg ? Fa : en ? on : Pa, i.uncache = 0, i;
}, Ne = function(t) {
  return (t = t.split(" "))[0] + " " + t[1];
}, ri = function(t, e, i) {
  var r = _(e);
  return z(parseFloat(e) + parseFloat(Nt(t, "x", i + "px", r))) + r;
}, Pa = function(t, e) {
  e.z = "0px", e.rotationY = e.rotationX = "0deg", e.force3D = 0, on(t, e);
}, qt = "0deg", ue = "0px", Yt = ") ", on = function(t, e) {
  var i = e || this, r = i.xPercent, n = i.yPercent, a = i.x, s = i.y, h = i.z, u = i.rotation, l = i.rotationY, g = i.rotationX, A = i.skewX, S = i.skewY, p = i.scaleX, k = i.scaleY, I = i.transformPerspective, w = i.force3D, f = i.target, C = i.zOrigin, d = "", c = w === "auto" && t && t !== 1 || w === !0;
  if (C && (g !== qt || l !== qt)) {
    var J = parseFloat(l) * ae, m = Math.sin(J), Q = Math.cos(J), y;
    J = parseFloat(g) * ae, y = Math.cos(J), a = ri(f, a, m * y * -C), s = ri(f, s, -Math.sin(J) * -C), h = ri(f, h, Q * y * -C + C);
  }
  I !== ue && (d += "perspective(" + I + Yt), (r || n) && (d += "translate(" + r + "%, " + n + "%) "), (c || a !== ue || s !== ue || h !== ue) && (d += h !== ue || c ? "translate3d(" + a + ", " + s + ", " + h + ") " : "translate(" + a + ", " + s + Yt), u !== qt && (d += "rotate(" + u + Yt), l !== qt && (d += "rotateY(" + l + Yt), g !== qt && (d += "rotateX(" + g + Yt), (A !== qt || S !== qt) && (d += "skew(" + A + ", " + S + Yt), (p !== 1 || k !== 1) && (d += "scale(" + p + ", " + k + Yt), f.style[G] = d || "translate(0, 0)";
}, Fa = function(t, e) {
  var i = e || this, r = i.xPercent, n = i.yPercent, a = i.x, s = i.y, h = i.rotation, u = i.skewX, l = i.skewY, g = i.scaleX, A = i.scaleY, S = i.target, p = i.xOrigin, k = i.yOrigin, I = i.xOffset, w = i.yOffset, f = i.forceCSS, C = parseFloat(a), d = parseFloat(s), c, J, m, Q, y;
  h = parseFloat(h), u = parseFloat(u), l = parseFloat(l), l && (l = parseFloat(l), u += l, h += l), h || u ? (h *= ae, u *= ae, c = Math.cos(h) * g, J = Math.sin(h) * g, m = Math.sin(h - u) * -A, Q = Math.cos(h - u) * A, u && (l *= ae, y = Math.tan(u - l), y = Math.sqrt(1 + y * y), m *= y, Q *= y, l && (y = Math.tan(l), y = Math.sqrt(1 + y * y), c *= y, J *= y)), c = z(c), J = z(J), m = z(m), Q = z(Q)) : (c = g, Q = A, J = m = 0), (C && !~(a + "").indexOf("px") || d && !~(s + "").indexOf("px")) && (C = Nt(S, "x", a, "px"), d = Nt(S, "y", s, "px")), (p || k || I || w) && (C = z(C + p - (p * c + k * m) + I), d = z(d + k - (p * J + k * Q) + w)), (r || n) && (y = S.getBBox(), C = z(C + r / 100 * y.width), d = z(d + n / 100 * y.height)), y = "matrix(" + c + "," + J + "," + m + "," + Q + "," + C + "," + d + ")", S.setAttribute("transform", y), f && (S.style[G] = y);
}, Ha = function(t, e, i, r, n) {
  var a = 360, s = j(n), h = parseFloat(n) * (s && ~n.indexOf("rad") ? bt : 1), u = h - r, l = r + u + "deg", g, A;
  return s && (g = n.split("_")[1], g === "short" && (u %= a, u !== u % (a / 2) && (u += u < 0 ? a : -a)), g === "cw" && u < 0 ? u = (u + a * er) % a - ~~(u / a) * a : g === "ccw" && u > 0 && (u = (u - a * er) % a - ~~(u / a) * a)), t._pt = A = new ot(t._pt, e, i, r, u, da), A.e = l, A.u = "deg", t._props.push(i), A;
}, hr = function(t, e) {
  for (var i in e)
    t[i] = e[i];
  return t;
}, Ta = function(t, e, i) {
  var r = hr({}, i._gsap), n = "perspective,force3D,transformOrigin,svgOrigin", a = i.style, s, h, u, l, g, A, S, p;
  r.svg ? (u = i.getAttribute("transform"), i.setAttribute("transform", ""), a[G] = e, s = Qe(i, 1), Wt(i, G), i.setAttribute("transform", u)) : (u = getComputedStyle(i)[G], a[G] = e, s = Qe(i, 1), a[G] = u);
  for (h in Ut)
    u = r[h], l = s[h], u !== l && n.indexOf(h) < 0 && (S = _(u), p = _(l), g = S !== p ? Nt(i, h, u, p) : parseFloat(u), A = parseFloat(l), t._pt = new ot(t._pt, s, h, g, A - g, Si), t._pt.u = p || 0, t._props.push(h));
  hr(s, r);
};
st("padding,margin,Width,Radius", function(o, t) {
  var e = "Top", i = "Right", r = "Bottom", n = "Left", a = (t < 3 ? [e, i, r, n] : [e + n, e + i, r + i, r + n]).map(function(s) {
    return t < 2 ? o + s : "border" + s + o;
  });
  We[t > 1 ? "border" + o : o] = function(s, h, u, l, g) {
    var A, S;
    if (arguments.length < 4)
      return A = a.map(function(p) {
        return Ot(s, p, u);
      }), S = A.join(" "), S.split(A[0]).length === 5 ? A[0] : S;
    A = (l + "").split(" "), S = {}, a.forEach(function(p, k) {
      return S[p] = A[k] = A[k] || A[(k - 1) / 2 | 0];
    }), s.init(h, S, g);
  };
});
var hn = {
  name: "css",
  register: pi,
  targetTest: function(t) {
    return t.style && t.nodeType;
  },
  init: function(t, e, i, r, n) {
    var a = this._props, s = t.style, h = i.vars.startAt, u, l, g, A, S, p, k, I, w, f, C, d, c, J, m, Q, y;
    Hi || pi(), this.styles = this.styles || tn(t), Q = this.styles.props, this.tween = i;
    for (k in e)
      if (k !== "autoRound" && (l = e[k], !(gt[k] && Vr(k, e, i, r, t, n)))) {
        if (S = typeof l, p = We[k], S === "function" && (l = l.call(i, r, t, n), S = typeof l), S === "string" && ~l.indexOf("random(") && (l = Je(l)), p)
          p(this, t, k, l, i) && (m = 1);
        else if (k.substr(0, 2) === "--")
          u = (getComputedStyle(t).getPropertyValue(k) + "").trim(), l += "", Rt.lastIndex = 0, Rt.test(u) || (I = _(u), w = _(l), w ? I !== w && (u = Nt(t, k, u, w) + w) : I && (l += I)), this.add(s, "setProperty", u, l, r, n, 0, 0, k), a.push(k), Q.push(k, 0, s[k]);
        else if (S !== "undefined") {
          if (h && k in h ? (u = typeof h[k] == "function" ? h[k].call(i, r, t, n) : h[k], j(u) && ~u.indexOf("random(") && (u = Je(u)), _(u + "") || u === "auto" || (u += St.units[k] || _(Ot(t, k)) || ""), (u + "").charAt(1) === "=" && (u = Ot(t, k))) : u = Ot(t, k), A = parseFloat(u), f = S === "string" && l.charAt(1) === "=" && l.substr(0, 2), f && (l = l.substr(2)), g = parseFloat(l), k in Bt && (k === "autoAlpha" && (A === 1 && Ot(t, "visibility") === "hidden" && g && (A = 0), Q.push("visibility", 0, s.visibility), Tt(this, s, "visibility", A ? "inherit" : "hidden", g ? "inherit" : "hidden", !g)), k !== "scale" && k !== "transform" && (k = Bt[k], ~k.indexOf(",") && (k = k.split(",")[0]))), C = k in Ut, C) {
            if (this.styles.save(k), y = l, S === "string" && l.substring(0, 6) === "var(--") {
              if (l = ct(t, l.substring(4, l.indexOf(")"))), l.substring(0, 5) === "calc(") {
                var B = t.style.perspective;
                t.style.perspective = l, l = ct(t, "perspective"), B ? t.style.perspective = B : Wt(t, "perspective");
              }
              g = parseFloat(l);
            }
            if (d || (c = t._gsap, c.renderTransform && !e.parseTransform || Qe(t, e.parseTransform), J = e.smoothOrigin !== !1 && c.smooth, d = this._pt = new ot(this._pt, s, G, 0, 1, c.renderTransform, c, 0, -1), d.dep = 1), k === "scale")
              this._pt = new ot(this._pt, c, "scaleY", c.scaleY, (f ? re(c.scaleY, f + g) : g) - c.scaleY || 0, Si), this._pt.u = 0, a.push("scaleY", k), k += "X";
            else if (k === "transformOrigin") {
              Q.push(ht, 0, s[ht]), l = Ka(l), c.svg ? Ii(t, l, 0, J, 0, this) : (w = parseFloat(l.split(" ")[2]) || 0, w !== c.zOrigin && Tt(this, c, "zOrigin", c.zOrigin, w), Tt(this, s, k, Ne(u), Ne(l)));
              continue;
            } else if (k === "svgOrigin") {
              Ii(t, l, 1, J, 0, this);
              continue;
            } else if (k in an) {
              Ha(this, c, k, A, f ? re(A, f + l) : l);
              continue;
            } else if (k === "smoothOrigin") {
              Tt(this, c, "smooth", c.smooth, l);
              continue;
            } else if (k === "force3D") {
              c[k] = l;
              continue;
            } else if (k === "transform") {
              Ta(this, l, t);
              continue;
            }
          } else k in s || (k = fe(k) || k);
          if (C || (g || g === 0) && (A || A === 0) && !Sa.test(l) && k in s)
            I = (u + "").substr((A + "").length), g || (g = 0), w = _(l) || (k in St.units ? St.units[k] : I), I !== w && (A = Nt(t, k, u, w)), this._pt = new ot(this._pt, C ? c : s, k, A, (f ? re(A, f + g) : g) - A, !C && (w === "px" || k === "zIndex") && e.autoRound !== !1 ? Ca : Si), this._pt.u = w || 0, C && y !== l ? (this._pt.b = u, this._pt.e = y, this._pt.r = Ia) : I !== w && w !== "%" && (this._pt.b = u, this._pt.r = pa);
          else if (k in s)
            Oa.call(this, t, k, u, f ? f + l : l);
          else if (k in t)
            this.add(t, k, u || t[k], f ? f + l : l, r, n);
          else if (k !== "parseTransform") {
            Qi(k, l);
            continue;
          }
          C || (k in s ? Q.push(k, 0, s[k]) : typeof t[k] == "function" ? Q.push(k, 2, t[k]()) : Q.push(k, 1, u || t[k])), a.push(k);
        }
      }
    m && Zr(this);
  },
  render: function(t, e) {
    if (e.tween._time || !Ti())
      for (var i = e._pt; i; )
        i.r(t, i.d), i = i._next;
    else
      e.styles.revert();
  },
  get: Ot,
  aliases: Bt,
  getSetter: function(t, e, i) {
    var r = Bt[e];
    return r && r.indexOf(",") < 0 && (e = r), e in Ut && e !== ht && (t._gsap.x || Ot(t, "x")) ? i && tr === i ? e === "scale" ? ya : ma : (tr = i || {}) && (e === "scale" ? Qa : Ba) : t.style && !wi(t.style[e]) ? Ja : ~e.indexOf("-") ? wa : Pi(t, e);
  },
  core: {
    _removeProperty: Wt,
    _getMatrix: Ri
  }
};
lt.utils.checkPrefix = fe;
lt.core.getStyleSaver = tn;
(function(o, t, e, i) {
  var r = st(o + "," + t + "," + e, function(n) {
    Ut[n] = 1;
  });
  st(t, function(n) {
    St.units[n] = "deg", an[n] = 1;
  }), Bt[r[13]] = o + "," + t, st(i, function(n) {
    var a = n.split(":");
    Bt[a[1]] = r[a[0]];
  });
})("x,y,z,scale,scaleX,scaleY,xPercent,yPercent", "rotation,rotationX,rotationY,skewX,skewY", "transform,transformOrigin,svgOrigin,force3D,smoothOrigin,transformPerspective", "0:translateX,1:translateY,2:translateZ,8:rotate,8:rotationZ,8:rotateZ,9:rotateX,10:rotateY");
st("x,y,z,top,right,bottom,left,width,height,fontSize,padding,margin,perspective", function(o) {
  St.units[o] = "px";
});
lt.registerPlugin(hn);
var Li = lt.registerPlugin(hn) || lt;
Li.core.Tween;
function Ea(o) {
  var t = this;
  this.x = 0, this.y = 0, this.z = -200, this.offsetX = 400, this.offsetY = 400, this.focalLength = 300, this.focalPower = 0.3, this.fov = 1e3, this.fog = { start: 2e4, end: 22e3, range: 2e3 }, this.moveTo = function(i, r, n) {
    var a = r || 0.5, s = {
      ease: n || "power1.out",
      onUpdate: function() {
        t.stage.root.changed = !0, t.onChange();
      }
    };
    for (var h in i)
      switch (h) {
        case "x":
        case "y":
        case "z":
          isNaN(i[h]) ? i[h] = 0 : i[h] = Math.round(i[h]);
        case "offsetX":
        case "offsetY":
        case "focalLength":
        case "focalPower":
        case "ease":
          s[h] = i[h];
          break;
        case "time":
          a = i[h];
          break;
      }
    var u = Li;
    u.to(t, a, s);
  }, this.moveToElement = function(i, r, n, a) {
    t.moveTo(
      {
        x: i._x + i._width * 0.5,
        y: i._y + i._height * 0.5,
        z: i._z - (isNaN(r) ? t.focalLength : r)
      },
      n,
      a
    );
  }, this.zFactor = function(i) {
    return i === 0 ? 1 : Math.pow(t.focalLength / i, t.focalPower);
  }, this.onChange = function() {
  };
  for (var e in o)
    this[e] = o[e];
}
function ke(o) {
  var t = this;
  this.className = "UIEvent", this.currentTarget = !1, typeof o == "string" && (o = { type: o, phase: "bubble" }), this._stopPropagation = !1, this.stopPropagation = function() {
    t._stopPropagation = !0;
  };
  for (var e in o)
    this[e] = o[e];
}
var Ra = 0;
class ln {
  constructor(t) {
    console.log("make");
    var e = this;
    this.id = "", this.___uniqueid = Ra++, this.x = 0, this.y = 0, this.z = 0, this.width = 0, this.height = 0, this.scale = 1, this.visible = !0, this.alpha = 1, this.rotation = 0, this.rotationX = 0, this.rotationY = 0, this._x = 0, this._y = 0, this._z = 0, this._width = 0, this._height = 0, this._scale = 1, this._visible = !0, this._alpha = 1, this._rotation = 0, this.__x = 0, this.__y = 0, this.__z = 0, this.__width = 0, this.__height = 0, this.viewport = !1, this.mouseEnabled = !0, this.smoothing = !1, this.onstage = !0, this.parent = !1, this.children = [], this.parentChain = [], this.__remove = !1;
    var i = {};
    this.root = e, this.rootArray = [];
    function r(d) {
      e.rootArray.push(d), s();
    }
    function n(d) {
      d.sort(function(c, J) {
        return c._z < J._z ? -1 : c._z > J._z ? 1 : 0;
      });
    }
    var a = !1;
    function s(d) {
      d === !0 ? (e.root.rootArray.sort(function(c, J) {
        return c._z < J._z ? -1 : c._z > J._z ? 1 : c.___uniqueid > J.___uniqueid ? -1 : c.___uniqueid < J.___uniqueid ? 1 : 0;
      }), a = !1) : a === !1 && (a = setTimeout(function() {
        s(!0);
      }, 1));
    }
    function h() {
      n(e.children);
    }
    function u(d, c) {
      return this.mouseEnabled === !0 && this._visible === !0 && this.onstage === !0 && d > this.__x && d < this.__x + this.__width && c > this.__y && c < this.__y + this.__height;
    }
    function l(d) {
      return d.parent = e, d.root = e.root, d.parentChain = e.parentChain.concat([e]), d.children.forEach((c) => {
        c.root = e.root;
      }), d.viewport === !1 && (d.viewport = e.viewport), e.children.push(d), h(), e.root.addRootNode && e.root.addRootNode(d), d;
    }
    function g(d) {
      var c = d.parent.children;
      if (c) {
        for (var J = c.length; J--; )
          if (c[J] == d) {
            c.splice(J, 1), d.__remove = !0, d.parent = !1;
            break;
          }
      }
    }
    function A() {
      g(e);
    }
    function S(d) {
      for (var c = 0; c < e.children.length; c++)
        if (e.children[c].id === d)
          return e.children[c];
      return !1;
    }
    function p(d, c) {
      return i[d] || (i[d] = []), i[d].push(c), e.mouseEnabled = !0, e;
    }
    function k(d, c) {
      if (i[d])
        if (c)
          for (var J = i[d].length; J--; )
            i[d][J] === c && i[d].splice(J, 1);
        else
          delete i[d];
      return e;
    }
    function I(d, c) {
      if (d.className != "UIEvent" && (d = new ke(d)), d.currentTarget === !1 && (d.currentTarget = e), d.target = e, i[d.type])
        for (var J = i[d.type].length; J--; )
          i[d.type][J].call(e, d);
      d.type === D.DOWN && e.draggable === !0 && (e.mouseData = { down: { x: e.x, y: e.y, z: e.z } }, e.root.dragTarget !== e && (e.trigger(D.DRAG_START), e.root.dragTarget = e), e.smartDrag !== !0 && d.stopPropagation()), c !== !1 && d._stopPropagation === !1 && e.parent !== !1 && e.parent.trigger(d);
    }
    function w(d) {
      return d.type in i && i[d.type] && i[d.type].length > 0;
    }
    function f(d) {
      if (d) {
        for (var c in d)
          e[c] = d[c];
        if (typeof e.root == "object" && (e.root.changed = !0), "image" in d && e.img && (e.img.src = e.image), "z" in d && e.trigger({ type: "zchange" }), "viewport" in d)
          for (var J = 0; J < e.children.length; J++)
            e.children.setProperty({ viewport: e });
      }
    }
    function C() {
      e.onstage === !0 && e.render.call(e);
    }
    this.update = function() {
    }, this.render = function() {
    }, this._render = C, this.trigger = I, this.willTrigger = w, this.on = p, this.off = k, this.setProperty = f, this.addChild = l, this.removeChild = g, this.remove = A, this.hitTest = u, this.getElementById = S, f(t), this.root === !0 && (this.root = this, this.addRootNode = r, this.sortRootNodes = s), this.viewport === !0 && (this.viewport = this);
  }
}
var D = {
  DRAG_START: "drag_start",
  DRAG: "drag",
  DRAG_EASE: "drag_ease",
  DRAG_END: "drag_end",
  WHEEL: "wheel",
  MOVE: "move",
  OVER: "over",
  OUT: "out",
  DOWN: "down",
  UP: "up",
  ENTER: "enter",
  LEAVE: "leave",
  ENTER_STAGE: "enter_stage",
  LEAVE_STAGE: "leave_stage",
  CLICK: "click",
  REMOVED: "removed",
  CHANGED: "changed",
  UPDATED: "updated",
  PROGRESS: "progress"
};
function La(o) {
  var t = this;
  this.currentTarget = !1, this.isDown = !1, this.position = { target: null, x: 0, y: 0 }, this.down = { target: null, x: 0, y: 0 }, this.move = { target: null, x: 0, y: 0 }, this.up = { target: null, x: 0, y: 0 }, this.vector = { x: 0, y: 0 }, this.data = {}, this.onAfterClick = !1;
  var e = o.root, i = o.target;
  this.config = o;
  var r = !1, n = "ontouchstart" in window && !/hp-tablet/gi.test(navigator.appVersion);
  function a(g) {
    if (g.multitouch = !1, g.zoomedIn = document.width / window.innerWidth > 1, n) {
      var A = g.originalEvent ? g.originalEvent.touches : g.touches;
      A && (A && A.length > 0 ? (g.pageX = A[0].pageX, g.pageY = A[0].pageY, g.multitouch = A.length > 1) : g.changedTouches && g.changedTouches.length > 0 && (g.pageX = g.changedTouches[0].pageX, g.pageY = g.changedTouches[0].pageY, g.multitouch = g.changedTouches.length > 1));
    }
  }
  function s(g) {
    var A = {
      mousedown: D.DOWN,
      mouseup: D.UP,
      mousemove: t.isDown ? D.DRAG : D.MOVE,
      mouseleave: D.LEAVE,
      mousewheel: D.WHEEL,
      touchstart: D.DOWN,
      touchend: D.UP,
      touchmove: t.isDown ? D.DRAG : D.MOVE,
      touchcancel: D.LEAVE,
      pointerdown: D.DOWN,
      pointerup: D.UP,
      pointermove: t.isDown ? D.DRAG : D.MOVE,
      pointerleave: D.LEAVE
    };
    return A[g] || "";
  }
  function h(g) {
    g.preventDefault(), a(g);
    var A = i.getBoundingClientRect(), S = {
      top: A.top + document.body.scrollTop,
      left: A.left + document.body.scrollLeft
    };
    t.position.x = g.pageX - S.left, t.position.y = g.pageY - S.top;
    var p = new ke({ type: s(g.type), mouse: t, pageX: t.position.x, pageY: t.position.y });
    switch (p.type) {
      case D.WHEEL:
        p.deltaFactor = g.originalEvent && g.originalEvent.deltaFactor ? g.originalEvent.deltaFactor : 1, p.deltaX = "deltaX" in g ? g.deltaX : g.originalEvent.deltaX, p.deltaY = "deltaY" in g ? g.deltaY : g.originalEvent.deltaY;
        break;
      case D.DOWN:
        t.isDown = !0, t.down.x = t.position.x, t.down.y = t.position.y, t.vector.x = 0, t.vector.y = 0, t.vector.length = 0, t.vector.maxLength = 0;
        break;
      case D.MOVE:
        break;
      case D.DRAG:
        t.vector.x = t.position.x - t.down.x, t.vector.y = t.position.y - t.down.y, t.vector.length = Math.sqrt(Math.pow(t.vector.x, 2) + Math.pow(t.vector.y, 2)), t.vector.maxLength = Math.max(t.vector.length, t.vector.maxLength);
        break;
      case D.LEAVE:
      case D.UP:
        t.isDown === !0 && (t.isDown = !1);
        break;
    }
    if (t.isDown === !1) {
      var k = new ke({ type: D.OVER, mouse: t, pageX: t.position.x, pageY: t.position.y }), I = u(k);
      if (I !== null && r !== I) {
        if (r !== !1) {
          i.classList.remove("cursor-pointer");
          var w = new ke({ type: D.OUT, mouse: t, pageX: t.position.x, pageY: t.position.y });
          r.trigger(w);
        }
        I.willTrigger(k) && (I.trigger(k), i.classList.add("cursor-pointer")), r = I;
      }
    }
    var f = u(p);
    if (f = l(f, p), f.trigger(p), f.willTrigger({ type: D.CLICK }) && i.classList.add("cursor-pointer"), p._stopPropagation === !1 && p.type === D.UP && t.vector.maxLength < 3) {
      var C = new ke({ type: D.CLICK, mouse: t, pageX: t.position.x, pageY: t.position.y });
      f.trigger(C), t.onAfterClick !== !1 && t.onAfterClick(f);
    }
  }
  function u(g) {
    for (var A = e, S = 0; S < e.rootArray.length; S++) {
      var p = e.rootArray[S];
      if (p.hitTest(g.pageX, g.pageY) && (p.viewport === !1 || p.viewport.hitTest(g.pageX, g.pageY))) {
        A = p;
        break;
      }
    }
    return A;
  }
  function l(g, A) {
    if (g.children.length > 0)
      for (var S = 0; S < g.children.length; S++) {
        var p = g.children[S];
        p.hitTest(A.pageX, A.pageY) && (g = l(p, A));
      }
    return g;
  }
  i.addEventListener("mousewheel", h, { passive: !1 }), i.addEventListener("pointerdown", h, { passive: !1 }), i.addEventListener("pointermove", h, { passive: !1 }), i.addEventListener("pointerup", h, { passive: !1 }), i.addEventListener("pointerleave", h, { passive: !1 });
}
class Wa {
  constructor(t) {
    var e = this;
    this.dragX = !0, this.dragY = !0, this.dragZ = !0, this.userBounds = new qi(), this.cameraBounds = new qi(), this.maxWidth = 1 / 0, this.maxHeight = 1 / 0, this.dragFactor = 1;
    var i = Li, r = Object.assign(
      {
        width: 100,
        height: 100,
        wheelZoom: !0,
        mouseEnabledX: !0,
        mouseEnabledY: !0,
        mouseEnabledZ: !0,
        fillStyle: "#FFF",
        fullscreen: !0
      },
      t
    );
    t.userBounds && this.userBounds.set(t.userBounds), t.cameraBounds && this.cameraBounds.set(t.cameraBounds), t.maxWidth && (this.maxWidth = t.maxWidth), t.maxHeight && (this.maxHeight = t.maxHeight), this.camera = new Ea({ stage: this });
    var n = this.camera, a = this.cvs = document.createElement("canvas"), s = this.ctx = a.getContext("2d", { alpha: !0 }), h = {
      width: 0,
      height: 0
    }, u;
    function l(f) {
      if (f !== !0) {
        u && clearTimeout(u), u = setTimeout(function() {
          l(!0);
        }, 50);
        return;
      }
      h.width = window.innerWidth, h.height = window.innerHeight, r.fullscreen === !0 && e.setSize(h.width, h.height);
    }
    this.root = new ln({
      root: !0,
      id: "stage",
      width: r.width,
      height: r.height,
      ctx: s,
      camera: e.camera,
      parent: !1,
      changed: !0
    }), this.mouse = r.mouseEnabled !== !1 ? new La({ target: a, root: this.root }) : !1;
    var g = {};
    function A() {
      return e.root.rootArray.filter(function(f) {
        return f.onstage === !0 && f.visible === !0;
      });
    }
    function S(f) {
      if (g[f])
        return g[f];
      for (var C = e.root.rootArray, d = C.length, c = 0; c < d; c++)
        if (C[c].id === f)
          return g[f] = C[c], C[c];
      return !1;
    }
    function p(f) {
      if (f.trigger(D.UPDATED, !1), f.beforeUpdate && f.beforeUpdate.call(f), f.physics === !0 && (f.x += f.vector.x, f.y += f.vector.y, f.z += f.vector.z, f.vector.x *= f.friction.x, f.vector.y *= f.friction.y, f.vector.z *= f.friction.z), f.position === "fixed") {
        if (f._onstage = !1, f._visible = f.visible, f._visible) {
          if (f.__x = f.right !== !1 ? f.root.width - f.width : f.x, f.__y = f.y, f.bottom !== !1)
            f.__y = f.root.height - (f.height + f.bottom);
          else if (f.verticalAlign !== !1)
            switch (f.verticalAlign) {
              case "middle":
                f.__y = (f.root.height - f.height) * 0.5;
                break;
              case "bottom":
                f.__y = f.root.height - f.height;
                break;
            }
          f._z = f.z, f._alpha = f.alpha, f.__width = f.width * f.scale, f.__height = f.height * f.scale, f.__right = f.__x + f.__width, f.__bottom = f.__y + f.__height, f.__x < f.root.width && f.__y < f.root.height && f.__right > 0 && f.__bottom > 0 && (f._onstage = !0);
        }
      } else if (f.parent) {
        var C = f.parent;
        if (f._visible = C._visible ? f.visible : !1, f.right !== !1 ? f._x = C._x + C.width - (f.width + f.right) : f._x = C._x + f.x, f._y = C._y + f.y, f._z = C._z + f.z, f._rotation = C._rotation + f.rotation, f._scale = C._scale * f.scale, f._alpha = C._alpha * f.alpha, f.__z = f._z - n.z, f._onstage = !1, f._visible === !0 && f.__z >= 0) {
          f.__lastZ !== f.__z && (f.zFactor = n.zFactor(f.__z), f.__lastZ = f.__z);
          var d = e.root;
          if (f.__x = n.offsetX + (f._x - n.x) * f.zFactor, (f.__x < d.width || f.repeatX === !0) && (f.__y = n.offsetY + (f._y - n.y) * f.zFactor, (f.__y < d.height || f.repeatY === !0) && (f._width = f._scale * f.width, f.__width = f._width * f.zFactor, (f.__x + f.__width > 0 || f.repeatX === !0) && (f._height = f._scale * f.height, f.__height = f._height * f.zFactor, (f.__y + f.__height > 0 || f.repeatY === !0) && (f._onstage = !0)))), f._onstage === !1 && f.plane === !0) {
            f.renderSkew();
            var c = f.hitRect;
            f._onstage = c.right > 0 && c.bottom > 0 && c.x < d.width && c.y < d.height;
          }
          if (f._onstage === !1 && f._rotation !== 0) {
            var c = {
              x: f.__x - f.__width * 1,
              y: f.__y - f.__height * 1,
              right: f.__x + f.__width * 2,
              bottom: f.__y + f.__height * 2
            };
            f._onstage = c.right > 0 && c.bottom > 0 && c.x < d.width && c.y < d.height;
          }
          f._onstage === !0 && n.fog.active === !0 && f.__z > n.fog.start && (f.__z < n.fog.end ? (n.fog.range = n.fog.end - n.fog.start, f._alpha *= (n.fog.end - f.__z) / n.fog.range) : f._alpha = 0);
        }
      }
      f.div !== !1 && f._onstage === !1 && (f.div.style.visibility = "hidden");
      for (var J = f.children, m = J.length; m--; )
        p(J[m]);
      f.changed = !1;
    }
    function k(f) {
      r.fillStyle = f, e.root.changed = !0;
    }
    function I() {
      var f = e.root, C = f.rootArray, d = C.length, c, J = [];
      for (s.clearRect(0, 0, a.width, a.height), s.fillStyle = r.fillStyle, s.fillRect(0, 0, a.width, a.height), p(f); d--; ) {
        if (c = C[d], c.onstage !== c._onstage && (c._onstage && c.willTrigger({ type: D.ENTER_STAGE }) ? c.trigger(D.ENTER_STAGE) : c.willTrigger({ type: D.LEAVE_STAGE }) && c.trigger(D.LEAVE_STAGE)), c.onstage = c._onstage, c.onstage) {
          if (c.willTrigger({ type: D.PROGRESS })) {
            var m = (c.__y + c.height) / (c.root.height + c.height);
            m != c.progress && (c.progress = m, c.trigger(D.PROGRESS));
          }
          c.smoothing === !1 && (c.__x = c.__x + 0.5 | 0, c.__y = c.__y + 0.5 | 0, c.__width = c.__width + 0.5 | 0, c.__height = c.__height + 0.5 | 0), c._render();
        }
        c.__remove === !0 && (J.unshift(d), c.trigger(D.REMOVED));
      }
      if (J.length > 0)
        for (d = J.length; d--; )
          C.splice(J[d], 1);
    }
    this.startRender = function() {
      i.ticker.add(I);
    }, this.stopRender = function() {
      i.ticker.remove(I), I();
    }, this.root.on("zchange", function(f) {
      this.sortRootNodes();
    }), this.root.dragTarget = !1, this.mouse && (this.root.on(D.DOWN, function(f) {
      this.mouseData = {
        down: {
          x: e.camera.x,
          y: e.camera.y
        }
      };
    }), this.root.on(D.DRAG, function(f) {
      var C = !1, d = this.dragTarget;
      if (this.dragTarget !== this && this.dragTarget !== !1) {
        var c = !1;
        if (d.smartDrag && e.dragY !== !1 && Math.abs(f.mouse.vector.x) < Math.abs(f.mouse.vector.y) ? e.camera.moveTo(e.userBounds.clamp({ y: this.mouseData.down.y - f.mouse.vector.y })) : c = !0, c) {
          var J = {
            x: d.mouseData.down.x + f.mouse.vector.x * (1 / d.zFactor),
            y: d.mouseData.down.y + f.mouse.vector.y * (1 / d.zFactor)
          };
          d.dragBounds && (d.dragBounds === "parent" && (d.dragBounds = {
            minX: Math.min(0, d.parent.width - d.width),
            maxX: Math.max(0, d.parent.width - d.width),
            minY: Math.min(0, d.parent.height - d.height),
            maxY: Math.max(0, d.parent.height - d.height)
          }), J.x = clamp(J.x, d.dragBounds.minX, d.dragBounds.maxX), J.y = clamp(J.y, d.dragBounds.minY, d.dragBounds.maxY)), i.to(d, {
            duration: 0.3,
            x: J.x,
            y: J.y,
            onUpdate: function() {
              d.trigger(D.DRAG_EASE);
            },
            onComplete: function() {
              d.trigger(D.DRAG_END);
            }
          });
        }
      } else
        C = !0;
      C === !0 && e.camera.moveTo(
        e.userBounds.clamp({
          x: this.mouseData.down.x - f.mouse.vector.x * e.dragFactor,
          y: this.mouseData.down.y - f.mouse.vector.y * e.dragFactor
        })
      );
    }), this.root.on(D.UP, function(f) {
      this.lastDragTarget = this.dragTarget, this.dragTarget && this.dragTarget.trigger(D.DRAG_END), this.dragTarget = !1;
    }), this.root.on(D.LEAVE, function(f) {
      this.lastDragTarget = this.dragTarget, this.dragTarget && this.dragTarget.trigger(D.DRAG_END), this.dragTarget = !1;
    }), this.root.on(D.WHEEL, function(f) {
      f.mouse.isDown && this.dragTarget !== this && this.dragTarget !== !1 ? this.dragTarget.setProperty({ z: this.dragTarget.z + f.deltaY * f.deltaFactor * 0.5 }) : r.wheelZoom && e.dragZ !== !1 ? e.camera.moveTo(e.userBounds.clamp({ z: e.camera.z + f.deltaY * f.deltaFactor * 1 })) : e.dragY !== !1 && e.camera.moveTo(e.userBounds.clamp({ y: e.camera.y - f.deltaY * f.deltaFactor * 2 }));
    })), this.setSize = function(f, C) {
      var d = Math.min(e.maxWidth, f), c = Math.min(e.maxHeight, C);
      a.width = d, a.height = c, e.camera.offsetX = Math.round(d * 0.5), e.camera.offsetY = Math.round(c * 0.5), e.root.width = d, e.root.height = c, d < f ? (e.cvs.style.left = "50%", e.cvs.style.marginLeft = Math.round(-d * 0.5) + "px") : (e.cvs.style.left = "", e.cvs.style.marginLeft = "");
    }, this.allImages = [], this.preloadImages = [], this.nonPreloadImages = [], this.importData = function(f) {
      w(f, e), setTimeout(function() {
        e.root.sortRootNodes();
      }, 1);
    };
    var w = function(f, C) {
      C == null && (C = e);
      var d = "";
      !f.id && f.image && (d = f.image.split("/").pop().split(".")[0]), f.image && (f.preload ? e.preloadImages.push(f.image) : e.nonPreloadImages.push(f.image), e.allImages.push(f.image)), window.assetsDirectory && f.image && f.image.indexOf(assetsDirectory) == -1 && f.image.indexOf("data:image") !== 0 && (f.image = assetsDirectory + f.image), C = C.addChild(new CanvasElement(Object.assign({ id: d, smoothing: !0 }, f, { children: [] })));
      for (var c in D) {
        var J = D[c];
        f[J] && (CanvasEventHandlers[f[J]] ? (C["__" + J] = f[J], C.on(J, CanvasEventHandlers[f[J]])) : typeof f[J] == "function" && (C["__" + J] = "INLINE", C.on(J, f[J])));
      }
      if (J = "update", f[J] && (CanvasEventHandlers[f[J]] ? (C["__" + J] = f[J], C.update = CanvasEventHandlers[f[J]]) : typeof f[J] == "function" && (C.update = f[J])), f.children)
        for (var m = 0; m < f.children.length; m++)
          w(f.children[m], C);
    };
    this.getElementById = S, this.getVisibleElements = A, this.addChild = this.root.addChild, this.on = this.root.on, this.trigger = this.root.trigger, this.setProperty = this.root.setProperty, this.render = I, this.setFillStyle = k, this.setSize(r.width, r.height), l(!0), window.addEventListener("resize", l), this.startRender();
  }
}
function Na() {
  var o = this.root.ctx;
  if (this.repeatX === !0)
    if (this.repeatY === !0)
      for (var t = this.root.width, e = this.root.height, i = this.__width, r = this.__height, n = this.__x % i - i, a = this.__y % r - r, s = n, h = a, u = Math.floor(i), l = Math.floor(r); h < e; ) {
        for (; s < t; )
          o.drawImage(this.bitmapData, s, h, i, r), s += u;
        s = n, h += l;
      }
    else
      for (var t = this.root.width, s = Math.floor(this.__x % this.__width - this.__width); s < t; )
        o.drawImage(this.bitmapData, s, this.__y, this.__width, this.__height), s += Math.floor(this.__width);
  else
    o.drawImage(this.bitmapData, this.__x, this.__y, this.__width, this.__height);
}
const lr = /* @__PURE__ */ (function() {
  var o = {}, t = [], e = {
    LOADING: 0,
    LOADED: 1
  };
  function i(l) {
    Array.isArray(l.images) || (l.images = [l.images]), Array.isArray(l.onCompleteParams) || (l.onCompleteParams = []), typeof l.onComplete != "function" && (l.onComplete = function() {
    }), l.images = l.images.filter(function(S) {
      return String(S).length > 0;
    });
    for (var g = 0; g < l.images.length; g++) {
      var A = String(l.images[g]);
      l.pattern && (A = l.pattern.replace("%IMAGE%", A)), l.root && (A = l.root + A), l.images[g] = A, s(A);
    }
    t.unshift(l), u();
  }
  function r() {
    var l = [];
    for (var g in o)
      l.push(g);
    return l;
  }
  function n(l) {
    return a(l) ? o[l] : !1;
  }
  function a(l) {
    return l in o;
  }
  function s(l) {
    if (a(l))
      u();
    else {
      var g = new Image();
      g.addEventListener("load", h, !1), g.setAttribute("orig_src", l), o[l] = { src: l, img: g, state: e.LOADING }, g.src = l;
    }
  }
  function h() {
    var l = this.getAttribute("orig_src");
    o[l] && (o[l].state = e.LOADED), u();
  }
  function u() {
    for (var l = t.length; l--; ) {
      for (var g = !0, A = [], S = 0; S < t[l].images.length; S++)
        if (o[t[l].images[S]].state == e.LOADING) {
          g = !1;
          break;
        } else {
          var p = o[t[l].images[S]];
          p.width = p.img.width, p.height = p.img.height, p.squareWidth = Math.min(p.width, p.height), p.squareX = (p.width - p.squareWidth) * 0.5, p.squareY = (p.height - p.squareWidth) * 0.5, A.push(p);
        }
      if (g) {
        var k = t.splice(l, 1);
        k[0].onComplete.apply(this, [A].concat(k[0].onCompleteParams || []));
      }
    }
  }
  return {
    load: i,
    getLoadedImages: r,
    getImage: n
  };
})();
function Ga() {
  if (this.imageLoaded !== !0 && this.imageLoading === !1) {
    if (this.image.indexOf("data:image") === 0) {
      var o = this, t = new Image();
      t.onload = function() {
        this.complete || console.log("INCOMPLETE LOAD"), ni([{ img: this }], o);
      }, t.src = this.image;
    } else {
      var t = lr.getImage(this.image);
      t ? ni([t], this) : lr.load({
        images: this.image,
        onComplete: ni,
        onCompleteParams: [this]
      });
    }
    this.imageLoading = !0;
  }
}
function ni(o, t) {
  t.imageLoading = !1, t.imageLoaded = !0, (t.autoSize === !0 || t.width === 0 || t.height === 0) && (t.width = o[0].width, t.height = o[0].height), t.bitmapData = o[0].img;
}
let Va = {};
function Ue(o) {
  return !isNaN(parseFloat(o));
}
function vt(o, t, e, i) {
  return typeof o == "boolean" ? Object.assign(t, e, i || {}) : Object.assign(t, e || {}, i || {});
}
function qa() {
  var o = this, t = o.html;
  if (t !== null && t !== "") {
    let r = function(n) {
      return Ue(n) && n > 0 ? n < 1 ? 1 : Math.floor(n) : 0;
    };
    o.bitmapData === !1 && (o.bitmapData = document.createElement("canvas"), o.bitmapDataContext = o.bitmapData.getContext("2d"), o.bitmapData.width = 10, o.bitmapData.height = 10);
    var e = Ya.render(t + "<br>", { width: o.width }, Va), i = {
      width: r(e.width),
      height: r(e.height)
    };
    o.bitmapData.width = i.width, o.bitmapData.height = i.height, o.width = i.width, o.height = i.height, i.width > 0 && i.height > 0 && o.bitmapDataContext.drawImage(e, 0, 0, i.width, i.height);
  }
}
var Ya = /* @__PURE__ */ (function(o) {
  var t = "_-_-_-_-_-", e;
  function i(l, g, A) {
    e = 0;
    var S = document.createElement("canvas"), p = g && Ue(g.scale) ? g.scale : 1;
    S.width = (g && Ue(g.width) ? g.width : 500) * p;
    var k = s("<html>" + l + "</html>"), I = vt(!0, u, A || {});
    return S.height = h(k, I, S, p, !0), h(k, I, S, p, !1), S;
  }
  function r(l) {
    vt(!0, u, l);
  }
  var n = {
    "font-size": "fontSize",
    "font-face": "fontFace",
    "font-weight": "fontWeight",
    "font-style": "fontStyle",
    color: "color",
    "line-height": "lineHeight",
    width: "width",
    "margin-top": "marginTop",
    "margin-left": "marginLeft",
    "margin-right": "marginRight",
    "margin-bottom": "marginBottom",
    "text-align": "textAlign",
    "text-decoration": "textDecoration",
    display: "display"
  };
  function a(l) {
    var g = {};
    if (l) {
      for (var A = l.split(";"), S = 0; S < A.length; S++)
        if (A[S].indexOf(":") > -1 && (A[S] = A[S].split(":"), n[A[S][0]])) {
          switch (n[A[S][0]]) {
            case "fontSize":
            case "marginTop":
            case "marginLeft":
            case "marginRight":
            case "marginBottom":
            case "lineHeight":
              A[S][1] = parseInt(A[S][1], 10);
              break;
          }
          g[n[A[S][0]]] = A[S][1];
        }
    }
    return g;
  }
  function s(l) {
    result = l.replace(/(\S{1}<)/g, function(w) {
      return w.slice(0, 1) + " <";
    }).replace(/(>\S{1})/g, function(w) {
      return "> " + w.slice(-1);
    }).replace(/(<.*?>)/g, function(w) {
      return w.replace(" ", t);
    }).split(/\s/g);
    for (var g = 0; g < result.length; g++)
      if (result[g].match(/<br>/))
        result[g] = { type: "html_break", html: !0 };
      else if (result[g].match(/<\w+.*?>/)) {
        var A = result[g].replace(t, " "), S = A.match(/<(\w{1,}\d{0,})\b/)[1], p = A.match(/class="(.*?)"/), k = A.match(/style="(.*?)"/), I = {};
        k && (I = a(k[1])), result[g] = { type: "html_open", html: !0, text: A, tag: S, inlineStyles: I, cls: p && p[1] };
      } else if (result[g].match(/<\/\w+.*?>/)) {
        var A = result[g].replace(t, " "), S = A.match(/<\/(\w{1,}\d{0,})\b/)[1];
        result[g] = { type: "html_close", html: !0, text: A, tag: S };
      } else
        result[g] = { type: "word", text: result[g] };
    for (var g = 0; g < result.length - 2; g++)
      result[g].t == "word" && result[g + 1].html && result[g + 2].t == "word" && result[g + 2].s.length == 1 && "!.:;?".indexOf(result[g + 2].s) != -1 && (result[g].s = result[g].s + result[g + 2].s, result.splice(g + 2, 1));
    return result;
  }
  function h(l, g, A, c, p) {
    var k = function(Y) {
      C.fillStyle = Y.color, C.font = Y.fontStyle + " " + Y.fontWeight + " " + Y.fontSize * c + "px " + Y.fontFace, d = C.measureText(" ").width;
    }, I = function(Y) {
      return g[Y] || {};
    }, w = function(Y, be, Me, ss) {
      var $t = {};
      return vt($t, Y, be, Me), $t._lineHeight = $t.lineHeight == -1 ? $t.fontSize * 1.15 : $t.lineHeight, $t;
    }, f = [vt({}, g.html)], C = A.getContext("2d", { alpha: f[0].backgroundColor == "none" || f[0].backgroundColor == "transparent" });
    f[0].backgroundColor != "none" && f[0].backgroundColor != "transparent" && (C.fillStyle = f[0].backgroundColor, C.fillRect(0, 0, A.width, A.height));
    var d = 0, c = c || 1, J = A.width, m = 0.85, Q = 0.15, y = [], B, x = !1, v = 0;
    function O() {
      B = { lineHeight: f[0]._lineHeight, width: 0, blocks: [] }, y.push(B), x = !1, Ue(B.lineHeight) && (e += B.lineHeight);
    }
    function P(Y) {
      var be = B.blocks.filter((Me) => Me.type == "text");
      x = vt({ firstTextBlock: Y.type == "text" && be.length == 0 }, Y), B.blocks.push(x);
    }
    O();
    for (var H = 0; H < l.length; H++) {
      var U = l[H];
      switch (U.type) {
        case "word":
          v = C.measureText(U.text).width, B.width + d + v > J && O(), x.type !== "text" ? P({ type: "text", text: [U.text], width: v }) : (x.text.push(U.text), x.width += d + v), B.width += d + v;
          break;
        case "html_open":
          f.unshift(w(f[0], I(U.tag), I(U.cls), U.inlineStyles)), k(f[0]), x.type == "style" ? x.style = vt({}, f[0]) : P({ type: "style", style: vt({}, f[0]) }), B.lineHeight = f[0]._lineHeight;
          break;
        case "html_close":
          f[0].display == "block" && O(), f.shift(), k(f[0]), x.type == "style" ? x.style = vt({}, f[0]) : P({ type: "style", style: vt({}, f[0]) }), B.lineHeight = f[0]._lineHeight;
          break;
        case "html_break":
          O();
          break;
      }
    }
    var F = y[0].blocks[0].style, V = 0, et = 0, mt = 0;
    function ft(Y) {
      return newVal = parseInt(Y), +newVal === newVal && !(newVal % 1) ? newVal : 0;
    }
    function tt(Y) {
      e += ft(Y.marginTop) + ft(Y.marginBottom) + ft(Y.paddingTop) + ft(Y.paddingBottom);
    }
    for (var H = 0; H < y.length; H++) {
      for (var ut = y[H], it = !1, It = 0, Vt = 0; Vt < ut.blocks.length; Vt++)
        ut.blocks[Vt].type === "text" && It++;
      for (var L = 0; L < ut.blocks.length; L++) {
        var q = ut.blocks[L];
        if (p)
          q.type === "style" && tt(q.style);
        else
          switch (q.type) {
            case "style":
              k(q.style), F = q.style, tt(q.style), (q.style.display !== "inline" || It < 2) && (et += q.style.marginTop + mt), mt = q.style.marginBottom;
              break;
            case "text":
              if (it || (et += ut.lineHeight * m, F.textAlign == "center" && (V = (J - ut.width) * 0.5, C.save()), it = !0), C.fillText(q.text.join(" "), V, et), F.textDecoration == "underline") {
                var Pt = Math.max(1, F.fontSize * c / 12);
                C.fillRect(V, et + Math.max(1.5, Math.ceil(Pt * 0.5)), q.width, Pt);
              }
              V += q.width;
              break;
          }
      }
      V = 0, et += ut.lineHeight * Q;
    }
    return e;
  }
  var u = {
    html: {
      fontSize: 12,
      fontFace: "Arial",
      fontWeight: "normal",
      fontStyle: "",
      color: "#000",
      lineHeight: -1,
      width: 500,
      marginTop: 0,
      marginLeft: 0,
      marginRight: 0,
      marginBottom: 0,
      maxWidth: 500,
      textAlign: "left",
      textDecoration: "none",
      display: "inline",
      backgroundColor: "none"
    },
    defaultStyle: {
      fontSize: 12,
      fontFace: "Arial",
      fontWeight: "normal",
      fontStyle: "",
      color: "#000",
      lineHeight: -1,
      width: 500,
      marginTop: 0,
      marginLeft: 0,
      marginRight: 0,
      marginBottom: 0,
      maxWidth: 500,
      textAlign: "left",
      textDecoration: "none",
      display: "inline"
    },
    b: {
      fontWeight: "bold",
      display: "inline"
    },
    strong: {
      fontWeight: "bold",
      display: "inline"
    },
    i: {
      fontStyle: "italic",
      display: "inline"
    },
    em: {
      fontStyle: "italic",
      display: "inline",
      marginBottom: 0,
      marginTop: 0
    },
    u: {
      textDecoration: "underline",
      display: "inline"
    },
    h1: {
      display: "block",
      fontWeight: "bold"
    },
    h2: {
      display: "block",
      fontWeight: "bold"
    },
    h3: {
      display: "block",
      fontWeight: "bold"
    },
    p: {
      display: "block"
    }
  };
  return {
    render: i,
    addStyles: r
  };
})();
function ba() {
  var o = this.root.ctx, t = !1, e = !1, i = !1;
  if (this.viewport !== !1 && (o.save(), t = !0), this.blendMode !== !1 && (o.globalCompositeOperation = this.blendMode), this._rotation !== 0) {
    e = !0;
    var r, n, a = this.parentChain.length;
    for (r = 0; r < a; r++)
      n = this.parentChain[r], n.rotation !== 0 && (n._tx = n.__x + n.__width * n.transformCenterX, n._ty = n.__y + n.__height * n.transformCenterY, o.translate(n._tx, n._ty), o.rotate(n.rotation * Vi), o.translate(-n._tx, -n._ty));
  }
  if (this.viewport !== !1) {
    var s = this.viewport, h = { x: s.__x, y: s.__y, right: s.__x + s.__width, bottom: s.__y + s.__height, width: s.__width, height: s.__height };
    o.beginPath(), o.rect(h.x, h.y, h.width, h.height), o.clip();
  }
  if (this.rotation !== 0 && (this._tx = this.__x + this.__width * this.transformCenterX, this._ty = this.__y + this.__height * this.transformCenterY, o.translate(this._tx, this._ty), o.rotate(this.rotation * Vi), o.translate(-this._tx, -this._ty)), this.clip && (o.beginPath(), o.rect(this.__x + this.clip.x, this.__y + this.clip.y, this.clip.width, this.clip.height), o.clip()), this._alpha < 1 && (i = !0, o.globalAlpha = this._alpha), this.fillStyle && (o.fillStyle = this.fillStyle, this.repeatX === !0 ? o.fillRect(0, this.__y, this.root.width, this.__height) : this.borderRadius !== !1 ? Sn(o, this.__x, this.__y, this.__width, this.__height, this.borderRadius, this.fillStyle) : o.fillRect(this.__x, this.__y, this.__width, this.__height)), this.bitmapData !== !1 ? Na.call(this) : this.html !== !1 ? qa.call(this) : this.image !== !1 && Ga.call(this), this.div !== !1) {
    var u = {
      position: "fixed",
      visibility: this._visible ? "visible" : "hidden",
      opacity: String(this._alpha),
      top: this.__y.toFixed(3) + "px",
      left: this.__x.toFixed(3) + "px",
      width: this.__width.toFixed(3) + "px",
      height: this.__height.toFixed(3) + "px"
    }, l = this.div.style, g = [];
    for (var A in u)
      l[A] !== u[A] && (l[A] = u[A], g.push(A));
  }
  t === !0 ? o.restore() : (this.blendMode !== !1 && (o.globalCompositeOperation = "source-over"), i === !0 && (o.globalAlpha = 1), e === !0 && o.setTransform(1, 0, 0, 1, 0, 0));
}
function Ge(o) {
  var t = new ln(
    Object.assign(
      {
        x: 0,
        y: 0,
        z: 0,
        width: 0,
        height: 0,
        scale: 1,
        alpha: 1,
        visible: !0,
        rotation: 0,
        transformCenterX: 0.5,
        transformCenterY: 0.5,
        _x: 0,
        _y: 0,
        _z: 0,
        _width: 0,
        _height: 0,
        _scale: 1,
        _alpha: 1,
        _visible: !0,
        _rotation: 0,
        __x: 0,
        __y: 0,
        __z: 0,
        __width: 0,
        __height: 0,
        __scale: 1,
        __visible: !0,
        right: !1,
        bottom: !1,
        verticalAlign: !1,
        onstage: !1,
        _onstage: !1,
        repeatX: !1,
        repeatY: !1,
        mouseEnabled: !!o.draggable,
        smoothing: !1,
        physics: null,
        vector: !1,
        html: !1,
        progress: 0,
        bitmapData: !1,
        clip: !1,
        imageLoaded: !1,
        imageLoading: !1,
        div: !1,
        borderRadius: !1,
        gradientFill: !1,
        blendMode: !1,
        zFactor: 1,
        render: ba
      },
      o
    )
  );
  if (t.vector !== !1 ? (t.alwaysRender = !0, t.physics = t.physics === null ? !0 : t.physics, t.vector = Object.assign({ x: 0, y: 0, z: 0 }, t.vector || {}), t.friction = Object.assign({ x: 1, y: 1, z: 1 }, t.friction || {})) : t.physics = !1, t.image ? (t.imageLoading = !1, t.imageLoaded = !1, t.width === 0 && (t.autoSize = !0)) : t.image = !1, t.bitmapData && (t.image = !0, t.img = t.bitmapData, t.imageLoading = !1, t.imageLoaded = !0, t.autoSize === !0 && (t.width = t.bitmapData.width, t.height = t.bitmapData.height)), t.linearGradient) {
    t.linearGradient.start = t.linearGradient.start || {}, t.linearGradient.start.x = t.linearGradient.start.x || 0, t.linearGradient.start.y = t.linearGradient.start.y || 0, t.linearGradient.end = t.linearGradient.end || {}, t.linearGradient.end.x = t.linearGradient.end.x || 0, t.linearGradient.end.y = t.height || 0, t.linearGradient.start || t.linearGradient.start, t.bitmapData = document.createElement("canvas"), t.bitmapData.width = t.width, t.bitmapData.height = t.height;
    for (var e = t.bitmapData.getContext("2d"), i = t.linearGradient, r = e.createLinearGradient(i.start.x, i.start.y, i.end.x, i.end.y), n = 0; n < i.steps.length; n++)
      r.addColorStop(i.steps[n].at, i.steps[n].color);
    e.fillStyle = r, e.fillRect(0, 0, t.width, t.height);
  } else if (t.radialGradient) {
    t.bitmapData = document.createElement("canvas"), t.bitmapData.width = t.width, t.bitmapData.height = t.height;
    for (var e = t.bitmapData.getContext("2d"), i = t.radialGradient, r = e.createRadialGradient(i.start.x, i.start.y, i.start.radius, i.end.x, i.end.y, i.end.radius), n = 0; n < i.steps.length; n++)
      r.addColorStop(i.steps[n].at, i.steps[n].color);
    e.fillStyle = r, e.fillRect(0, 0, t.width, t.height);
  }
  return t.html && (t.htmlLoaded = !1, t.bitmapData = !1), t.plane && window.CanvasPlaneInit && CanvasPlaneInit.call(t), t.setImage = function(a) {
    t.imageLoaded = !1, t.imageLoading = !1, t.image = a, t.bitmapData = !1;
  }, t.globalToLocal = function(a, s) {
    var h = Object.assign({}, a);
    if (s)
      var u = 1 / s.zFactor(t.z - s.z);
    else
      var u = 1 / t.zFactor;
    var l = t.root.width, g = t.root.height, A = -l * 0.5 * u, S = -g * 0.5 * u, p, k, I, w;
    "x" in h && (typeof h.x == "string" && h.x.indexOf("%") > -1 ? (h.x = parseFloat(h.x), p = l * (h.x / 100) * u) : (h.x = parseFloat(h.x), p = h.x * u), t.x = A + p, h.round === !0 && (t.x = Math.round(t.x))), s && (t.x += s.x), "y" in h && (typeof h.y == "string" && h.y.indexOf("%") > -1 ? (h.y = parseFloat(h.y), k = g * (h.y / 100) * u) : (h.y = parseFloat(h.y), k = h.y * u), t.y = S + k, h.round === !0 && (t.y = Math.round(t.y))), s && (t.y += s.y), "height" in h && (typeof h.height == "string" && h.height.indexOf("%") > -1 ? (h.height = parseFloat(h.height), w = g * (h.height / 100) * u) : (h.height = parseFloat(h.height), w = h.height * u), t.height = w, h.round === !0 && (t.height = Math.round(t.height))), "width" in h && (typeof h.width == "string" && h.width.indexOf("%") > -1 ? (h.width = parseFloat(h.width), I = l * (h.width / 100) * u) : (h.width = parseFloat(h.width), I = h.width * u), t.width = I, h.round === !0 && (t.width = Math.round(t.width)));
  }, t;
}
const Ma = "data:image/png;base64,", za = "data:image/png;base64,";
function b(o, t, e) {
  typeof t != "number" && (typeof t == "boolean" && (e = !0), isNaN(t) && (t = 0));
  var i = o + Math.random() * (t - o);
  return e === !0 ? Math.round(i) : i;
}
const Za = Math.PI / 180;
function ja(o) {
  var t = document.createElement("canvas"), e = t.getContext("2d");
  t.width = 48, t.height = 48;
  var s = e.createRadialGradient(24, 24, 2, 24, 24, 20);
  s.addColorStop(0, "#ffffffCC"), s.addColorStop(0.5, "#ffffffCC"), s.addColorStop(1, "#ffffff00"), e.fillStyle = s, e.fillRect(2, 2, 44, 44);
  var i = document.createElement("canvas");
  i.width = 1024, i.height = 1024;
  var r = i.getContext("2d"), n = document.createElement("canvas");
  n.width = 48, n.height = 48;
  var a = n.getContext("2d"), s = a.createLinearGradient(0, 0, 0, 48);
  s.addColorStop(0, "#ffffff00"), s.addColorStop(0.5, "#ffffffCC"), s.addColorStop(1, "#ffffff00"), a.fillStyle = s, a.fillRect(21, 0, 5, 88), a.save(), a.translate(0, 48), a.rotate(-90 * Za), a.fillRect(21, 0, 5, 88), a.translate(-24, -24), a.restore(), a.drawImage(t, 10, 10, 28, 28);
  var h, u, l, g;
  for (h = 0; h < 2e3; h++)
    u = b(1, 3), b(0.7, 1), l = b(i.width), g = b(i.height), r.globalAlpha = b(), r.drawImage(t, l, g, u, u);
  for (h = 0; h < 64; h++)
    u = b(3, 7), b(0.7, 1), l = b(i.width), g = b(i.height), r.globalAlpha = b(), r.drawImage(t, l, g, u, u);
  for (h = 0; h < 12; h++)
    u = b(10, 16), b(0.7, 1), l = b(i.width), g = b(i.height), r.globalAlpha = b(), r.drawImage(n, l, g, u, u);
  var A = o.camera.zFactor(5e4), S = new Ge({
    id: "stars",
    bitmapData: i,
    smoothing: !0,
    x: 0,
    y: 0,
    z: 5e4,
    width: window.innerWidth * (1 / A),
    height: window.innerWidth * (1 / A),
    scale: 1,
    repeatX: !0,
    repeatY: !0
  });
  return S;
}
var Ci = {
  CLICK: "click",
  UPDATED: "updated"
};
let nt;
function Xa() {
  nt = new Wa({
    fillStyle: "rgb(10,5,50)",
    mouseEnabled: !0,
    wheelZoom: !0,
    userBounds: {
      minY: -1e4,
      maxY: 1e4
    },
    fullscreen: !1
  });
}
function _a() {
  nt.addChild(ja(nt)).on(Ci.UPDATED, (t) => {
    t.target.y += 0.5;
  });
}
function fr(o) {
  this.alpha = b(0.9, 1), this.rotationShift += this.rotationSpeed, this.rotation = Math.sin(this.rotationShift) * 5;
}
function $a() {
  this.alpha = Math.random();
}
function ts(o) {
  nt.camera.moveToElement(o.target, 50, 3, Aa.easeInOut), fn.forEach((t) => t.selected = !1), o.target.selected = !0;
}
function es() {
  var o, t = 10, e = 2e3, i = 0.3, r = 1e4, n = (r - e) / t, a = 1 - i;
  for (o = 0; o < t; o++) {
    var s = o / t, h = a * s * 0.2;
    nt.addChild(
      new Ge({
        position: "fixed",
        x: 0,
        y: 0,
        z: 0,
        offSetZ: e + o * n,
        id: `fog${o}`,
        draggable: !1,
        smoothing: !1,
        width: window.innerWidth,
        height: window.innerHeight,
        fillStyle: `rgba(10,5,50,${h})`,
        beforeUpdate: function() {
          this.setProperty({ z: nt.camera.z + this.offSetZ });
        }
        // onResize: function () {
        //   this.setProperty({ width: window.innerWidth, height: window.innerHeight });
        // }
      })
    );
  }
}
const fn = [];
function is() {
  for (let t = 0; t < 500; t++) {
    var o = nt.addChild(
      new Ge({
        image: Ma,
        x: b(-3e3, 3e3),
        y: b(0, -3e3),
        z: b(-500, 15e3),
        draggable: !0,
        smoothing: !0,
        launched: !1,
        width: 100,
        height: 100,
        alpha: 0.9,
        rotation: b(-5, 5),
        rotationShift: 0,
        rotationSpeed: b(-0.05, 0.05),
        physics: !0,
        vector: { x: 0, y: b(0, -0.01), z: 0 },
        beforeUpdate: fr
      })
    ).on(Ci.UPDATED, fr).on(Ci.CLICK, ts);
    fn.push(o), o.addChild(
      new Ge({
        image: za,
        x: 10,
        y: 25,
        z: 0,
        scale: 4,
        width: 20,
        height: 20,
        transformCenterY: 1,
        beforeUpdate: $a
      })
    );
  }
}
function rs(o) {
  return Xa(), _a(), es(), is(), o.appendChild(nt.cvs), nt.cvs.addEventListener("pointerdown", ai, { passive: !1 }), nt.cvs.addEventListener("pointermove", ai, { passive: !1 }), nt.cvs.addEventListener("pointerup", ai, { passive: !1 }), nt;
}
function ai(o) {
  o.stopPropagation(), o.preventDefault();
}
function ns() {
  nt.stopRender(), nt.cvs.remove();
}
const as = (o) => {
  const t = Pe(null), { width: e, height: i } = cn({ ref: t }), [r, n] = ur(null);
  return Fe(() => {
    r && r.setSize(e, i);
  }, [e, i]), Fe(() => {
    if (t.current) {
      const a = rs(t.current);
      return a.setSize(e, i), n(a), () => {
        n(null), ns();
      };
    }
  }, []), /* @__PURE__ */ un("div", { ref: t, style: { width: 500, height: 500 }, ...o });
}, ls = as;
export {
  ls as LanternsReact
};
//# sourceMappingURL=index.js.map
